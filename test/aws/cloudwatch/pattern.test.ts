// https://github.com/aws/aws-cdk/blob/a2c633f1e698249496f11338312ab42bd7b1e4f0/packages/aws-cdk-lib/aws-logs/test/pattern.test.ts

import { FilterPattern } from "../../../src/aws/cloudwatch";
...
describe("pattern", () => {
  ...
  describe("json patterns", () => {
    ...
    test("combining with AND or OR", () => {
      ...
      expect('{ ($.field <= 300) && ($.field = "value") }').toEqual(
        andPattern.logPatternString,
      );

      const orPattern = FilterPattern.any(p1, p2);
      expect('{ ($.field <= 300) || ($.field = "value") }').toEqual(
        orPattern.logPatternString,
      );
    });
  ...
  describe("table patterns", () => {
    test("simple model", () => {
      const pattern = FilterPattern.spaceDelimited(
        "...",
        "status_code",
        "bytes",
      );
    ...
    test("add restrictions", () => {
      ...
      expect(pattern.logPatternString).toEqual(
        '[..., status_code = "4*" && status_code != 403, bytes]',
      );
    });
