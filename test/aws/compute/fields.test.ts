import { FieldUtils, JsonPath, TaskInput } from "../../../src/aws/compute";
...
describe("Fields", () => {
  ...
  (test("cannot have JsonPath fields in arrays", () => {
    ...
    test("datafield path must be correct", () => {
      ...
      expect(() => JsonPath.stringAt("$hello")).toThrowError(
        jsonPathValidationErrorMsg,
      );
      expect(() => JsonPath.stringAt("hello")).toThrowError(
        jsonPathValidationErrorMsg,
      );
      expect(() => JsonPath.stringAt("States.FooBar")).toThrowError(
        jsonPathValidationErrorMsg,
      );
    }),
  ...
  test("infinitely recursive object graphs do not break referenced path finding", () => {
    ...
    const paths = {
      ...
      listField: JsonPath.listAt("$.listField"),
      recursiveField: undefined as any,
      deep: ["literal", deepObject],
    };
    paths.recursiveField = paths;
    deepObject.recursiveField = paths;
    expect(FieldUtils.findReferencedPaths(paths)).toStrictEqual([
      "$.listField",
      "$.numField",
      "$.stringField",
    ]);
  ...
  test("rendering a non-object value should just return itself", () => {
    ...
    expect(FieldUtils.renderObject("Hello World" as any)).toEqual(
      "Hello World",
    );
    expect(FieldUtils.renderObject(null as any)).toEqual(null);
    expect(FieldUtils.renderObject(3.14 as any)).toEqual(3.14);
    expect(FieldUtils.renderObject(true as any)).toEqual(true);
    expect(FieldUtils.renderObject(undefined)).toEqual(undefined);
  });
...
describe("intrinsics constructors", () => {
  ...
  test("arrayPartition", () => {
    ...
    expect(
      FieldUtils.renderObject({
        Field: JsonPath.arrayPartition(
          JsonPath.listAt("$.inputArray"),
          JsonPath.numberAt("$.chunkSize"),
        ),
      }),
    ).toEqual({
  ...
  test("arrayContains", () => {
    ...
    expect(
      FieldUtils.renderObject({
        Field: JsonPath.arrayContains(
          JsonPath.listAt("$.inputArray"),
          JsonPath.numberAt("$.lookingFor"),
        ),
      }),
    ).toEqual({
  ...
  test("arrayRange", () => {
    ...
    expect(
      FieldUtils.renderObject({
        Field: JsonPath.arrayRange(
          JsonPath.numberAt("$.start"),
          JsonPath.numberAt("$.end"),
          JsonPath.numberAt("$.step"),
        ),
      }),
    ).toEqual({
  ...
  test("arrayGetItem", () => {
    ...
    expect(
      FieldUtils.renderObject({
        Field: JsonPath.arrayGetItem(
          JsonPath.numberAt("$.inputArray"),
          JsonPath.numberAt("$.index"),
        ),
      }),
    ).toEqual({
  ...
  test("hash", () => {
    ...
    expect(
      FieldUtils.renderObject({
        Field: JsonPath.hash(
          JsonPath.objectAt("$.Data"),
          JsonPath.stringAt("$.Algorithm"),
        ),
      }),
    ).toEqual({
  ...
  test("jsonMerge", () => {
    expect(
      FieldUtils.renderObject({
        Field: JsonPath.jsonMerge(
          JsonPath.objectAt("$.Obj1"),
          JsonPath.objectAt("$.Obj2"),
        ),
      }),
    ).toEqual({
  ...
  test("mathAdd", () => {
    ...
    expect(
      FieldUtils.renderObject({
        Field: JsonPath.mathAdd(
          JsonPath.numberAt("$.value1"),
          JsonPath.numberAt("$.step"),
        ),
      }),
    ).toEqual({
  ...
  test("stringSplit", () => {
    ...
    expect(
      FieldUtils.renderObject({
        Field: JsonPath.stringSplit(
          JsonPath.stringAt("$.inputString"),
          JsonPath.stringAt("$.splitter"),
        ),
      }),
    ).toEqual({
...
test("find task token even if nested in intrinsic functions", () => {
  expect(
    FieldUtils.containsTaskToken({ x: JsonPath.array(JsonPath.taskToken) }),
  ).toEqual(true);

  expect(FieldUtils.containsTaskToken({ x: JsonPath.array("nope") })).toEqual(
    false,
  );
  ...
  expect(
    FieldUtils.containsTaskToken({
      x: JsonPath.stringAt("States.Array($$.Task.Token)"),
    }),
  ).toEqual(true);
});
