// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-ecs/test/fargate/fargate-task-definition.test.ts

import { ecsTaskDefinition } from "@cdktn/provider-aws";
import { HttpBackend, Lazy, TerraformVariable, Testing } from "cdktn";
  ...
  describe("When creating a Fargate TaskDefinition", () => {
    ...
    test("support lazy cpu and memory values", () => {
      ...
      new ecs.FargateTaskDefinition(stack, "FargateTaskDef", {
        cpu: Lazy.numberValue({ produce: () => 128 }),
        memoryLimitMiB: Lazy.numberValue({ produce: () => 1024 }),
      });
    ...
    test("with all properties set", () => {
      ...
      Template.synth(stack).toHaveResourceWithProperties(
        ecsTaskDefinition.EcsTaskDefinition,
        {
          ...
          network_mode: "awsvpc",
          pid_mode: "task",
          requires_compatibilities: [ecs.LaunchType.FARGATE],
          runtime_platform: {
    ...
    test("throws when pidMode is not task", () => {
      ...
      expect(() => {
        ...
        /'pidMode' can only be set to 'task' for Linux Fargate containers, got: 'host'./,
      );
    });
  });
  ...
  describe("When importing from an existing Fargate TaskDefinition", () => {
    test("can succeed using TaskDefinition Arn", () => {
      ...
      const taskDefinition =
        ecs.FargateTaskDefinition.fromFargateTaskDefinitionArn(
          stack,
          "FARGATE_TD_ID",
    ...
    test("can succeed using attributes", () => {
      ...
      // WHEN
      const taskDefinition =
        ecs.FargateTaskDefinition.fromFargateTaskDefinitionAttributes(
          stack,
          "TD_ID",
          {
    ...
    test("Passing in token for ephemeral storage will not throw error", () => {
      ...
      // WHEN
      const param = new TerraformVariable(stack, "prammm", {
        type: "number",
        default: 1,
      });
      ...
      const taskDefinition = new ecs.FargateTaskDefinition(
        ...
        "FargateTaskDef",
        {
          ephemeralStorageGiB: param.numberValue,
        },
      );
...
// Wrapping describe with synth/snapshot coverage for the FargateTaskDefinition
// construct exercised above (harness idiom: test/aws/notify/queue.test.ts +
// test/aws/compute/ecs/base-service.test.ts "base-service synth").
describe("fargate task definition synth", () => {
  test("FargateTaskDefinition with all properties set should synth and match SnapShot", () => {
    // GIVEN
    const stack = new AwsStack();
