// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-ecs/test/container-definition.test.ts
...
describe("container definition", () => {
  describe("When creating a Task Definition", () => {
    ...
    describe("PortMap validates", () => {
      ...
      test("throws when PortMapping.containerPortRange is used along with PortMapping.hostPort", () => {
        ...
        // THEN
        expect(() => portMap.validate()).toThrow(
          'Cannot set "hostPort" while using a port range for the container.',
        );
      });
      ...
      test("throws when PortMapping.containerPortRange is not a concrete value", () => {
        ...
        const portMap = new ecs.PortMap(ecs.NetworkMode.AWS_VPC, {
          containerPort: ecs.ContainerDefinition.CONTAINER_PORT_USE_RANGE,
          containerPortRange: Lazy.stringValue({
            produce: () => "8080-8081",
          }),
        });
        ...
        // THEN
        expect(() => portMap.validate()).toThrow(
          "The value of containerPortRange must be concrete (no Tokens)",
        );
      });
    ...
    describe("ServiceConnect class", () => {
      describe("isServiceConnect", () => {
        test("return true if params has portname", () => {
          ...
          const networkmode = ecs.NetworkMode.AWS_VPC;
          const serviceConnect = new ecs.ServiceConnect(
            networkmode,
            portMapping,
          );
          // THEN
    ...
    test("add a container using all props", () => {
      ...
      // NEW_STYLE_STACK_SYNTHESIS_CONTEXT to force legacy (`AssetParameters...`) CFN asset
      // parameters instead of the new-style S3 asset publishing convention -- a CDK
      // CloudFormation-synthesizer concept with no TerraConstructs equivalent (there is no
      // alternate "stack synthesizer" for Terraform asset uploads). Dropped; see
      // `environment-file.test.ts` for the same deviation note.
      const stack = new AwsStack(Testing.app({ fakeCdktfJsonPath: true }));
      const taskDefinition = new ecs.Ec2TaskDefinition(stack, "TaskDef");
      const secret = new encryption.Secret(stack, "Secret");
      const linuxParameters = new ecs.LinuxParameters(stack, "LinuxParameters");
      const logging = new ecs.AwsLogDriver({ streamPrefix: "prefix" });
      new ecs.ContainerDefinition(stack, "Container", {
        image: ecs.ContainerImage.fromRegistry("/aws/aws-example-app"),
        taskDefinition,
        memoryLimitMiB: 1024,
        ...
        startTimeout: Duration.millis(2000),
        stopTimeout: Duration.millis(5000),
        user: "rootUser",
      ...
      expect(renderContainerDefinitions(stack)).toMatchObject([
        {
          ...
          dnsSearchDomains: ["example.com"],
          dnsServers: ["host.com"],
          dockerLabels: { key: "fooLabel", value: "barLabel" },
          dockerSecurityOptions: ["ECS_SELINUX_CAPABLE=true"],
          entryPoint: ["top", "-b"],
          environment: [
            { name: "key", value: "foo" },
          ...
          // asset-based value depends on the staged asset hash / logical ids; only the
          // shape is verified here (see `environment-file.test.ts` for the dedicated,
          // full-fidelity `EnvironmentFile.fromAsset` coverage).
          environmentFiles: [{ type: "s3" }],
          essential: true,
          extraHosts: [{ hostname: "name", ipAddress: "dev-db.hostname.pvt" }],
          healthCheck: {
          ...
          pseudoTerminal: true,
          readonlyRootFilesystem: true,
          resourceRequirements: [{ type: "GPU", value: "256" }],
          secrets: [
            { name: "SECRET", valueFrom: stack.resolve(secret.secretArn) },
          ],
          startTimeout: 2,
          stopTimeout: 5,
          systemControls: [{ namespace: "SomeNamespace", value: "SomeValue" }],
      ...
      //       DockerSecurityOptions: ['ECS_SELINUX_CAPABLE=true'],
      //       EntryPoint: ['top', '-b'],
      //       Environment: [{ Name: 'key', Value: 'foo' }, { Name: 'value', Value: 'bar' }],
      //       EnvironmentFiles: [{
      //         Type: 's3',
      //         Value: {
      //           'Fn::Join': ['', [
      ...
      //       Essential: true,
      //       ExtraHosts: [{ Hostname: 'name', IpAddress: 'dev-db.hostname.pvt' }],
      //       HealthCheck: { Command: ['CMD-SHELL', 'curl localhost:8000'], Interval: 30, Retries: 3, Timeout: 5 },
      //       Hostname: 'host.example.com',
      //       Image: '/aws/aws-example-app',
      ...
      //         LogDriver: 'awslogs',
      //         Options: {
      //           'awslogs-group': { Ref: 'ContainerLogGroupE6FD74A4' },
      //           'awslogs-stream-prefix': 'prefix',
      ...
      //       ReadonlyRootFilesystem: true,
      //       ResourceRequirements: [{ Type: 'GPU', Value: '256' }],
      //       Secrets: [{ Name: 'SECRET', ValueFrom: { Ref: 'SecretA720EF05' } }],
      //       StartTimeout: 2,
      //       StopTimeout: 5,
      //       SystemControls: [{ Namespace: 'SomeNamespace', Value: 'SomeValue' }],
      //       User: 'rootUser',
    ...
    describe("With network mode NAT", () => {
      test("produces undefined CF networkMode property", () => {
        ...
        // THEN
        const taskDefResource = new Template(stack).resourceTypeArray(
          ecsTaskDefinition.EcsTaskDefinition,
        )[0] as any;
        expect(taskDefResource.network_mode).toBeUndefined();
        // Template.fromStack(stack).hasResourceProperties('AWS::ECS::TaskDefinition', {
  ...
  test("docker labels should be absent if empty object is provided", () => {
    ...
    // THEN
    expect(renderContainerDefinitions(stack)[0].dockerLabels).toBeUndefined();
    // Template.fromStack(stack).hasResourceProperties('AWS::ECS::TaskDefinition', {
    //   ContainerDefinitions: [
    //     Match.objectLike({
    ...
    describe("with EC2 task definitions", () => {
      ...
      test("can add s3 bucket environment file to the container definition", () => {
        ...
        taskDefinition.addContainer("cont", {
          ...
          memoryLimitMiB: 1024,
          environmentFiles: [
            ecs.EnvironmentFile.fromBucket(bucket, "test-key"),
          ],
        });
        ...
        expect(renderContainerDefinitions(stack)).toMatchObject([
          {
            environmentFiles: [
              {
                type: "s3",
                value: `arn:${stack.resolve(stack.partition)}:s3:::${stack.resolve(bucket.bucketName)}/test-key`,
              },
            ],
          },
        ]);
        ...
        //         Type: 's3',
        //         Value: {
        //           'Fn::Join': ['', ['arn:', { Ref: 'AWS::Partition' }, ':s3:::', { Ref: 'Bucket83908E77' }, '/test-key']],
        //         },
        //       }],
        //     }),
        //   ],
  ...
  describe("Given InferenceAccelerator resource parameter", () => {
    test("correctly adds resource requirements to container definition using inference accelerator resource property", () => {
      ...
      // THEN
      // TERRACONSTRUCTS DEVIATION: the terraform-provider-aws `aws_ecs_task_definition`
      // resource has no `inference_accelerators` argument (confirmed absent from the
      // provider schema) -- see the deviation note on
      // `TaskDefinition._inferenceAccelerators` in `base/task-definition.ts`. The
      // top-level `Family`/`InferenceAccelerators` CFN assertion below has no Terraform
      // counterpart; only the container-level `resourceRequirements` (which IS rendered
      // into the jsonencoded `container_definitions`) is asserted here.
      expect(renderContainerDefinitions(stack)).toMatchObject([
        {
          image: "test",
          resourceRequirements: [
            { type: "InferenceAccelerator", value: "device1" },
          ],
        },
      ]);
      // Template.fromStack(stack).hasResourceProperties('AWS::ECS::TaskDefinition', {
      //   Family: 'Ec2TaskDef',
      //   InferenceAccelerators: [{ DeviceName: 'device1', DeviceType: 'eia2.medium' }],
      //   ContainerDefinitions: [
      //     Match.objectLike({
      //       Image: 'test',
      //       ResourceRequirements: [{ Type: 'InferenceAccelerator', Value: 'device1' }],
      //     }),
      //   ],
      // });
    });
    ...
    test("correctly adds resource requirements to container definition using both props and addInferenceAcceleratorResource method", () => {
      ...
      // THEN
      // TERRACONSTRUCTS DEVIATION: see the note above -- no `inference_accelerators`
      // Terraform field; only the container-level `resourceRequirements` is asserted.
      expect(renderContainerDefinitions(stack)).toMatchObject([
        {
          image: "test",
          resourceRequirements: [
            { type: "InferenceAccelerator", value: "device1" },
            { type: "InferenceAccelerator", value: "device2" },
          ],
        },
      ]);
      // Template.fromStack(stack).hasResourceProperties('AWS::ECS::TaskDefinition', {
      //   Family: 'Ec2TaskDef',
      //   InferenceAccelerators: [
      //     { DeviceName: 'device1', DeviceType: 'eia2.medium' },
      //     { DeviceName: 'device2', DeviceType: 'eia2.large' },
      //   ],
      //   ContainerDefinitions: [
      //     Match.objectLike({
      //       Image: 'test',
  ...
  test("can add secret environment variables to the container definition", () => {
    ...
    const container = taskDefinition.addContainer("cont", {
      ...
      secrets: {
        ...
        SECRET_ID: ecs.Secret.fromSecretsManagerVersion(secret, {
          versionId: "version-id",
        }),
        SECRET_STAGE: ecs.Secret.fromSecretsManagerVersion(secret, {
    ...
    expect(renderContainerDefinitions(stack)).toMatchObject([
      {
        secrets: [
          ...
          {
            name: "SECRET_ID",
            valueFrom: `${stack.resolve(secret.secretArn)}:::version-id`,
          },
          {
            name: "SECRET_STAGE",
            valueFrom: `${stack.resolve(secret.secretArn)}::version-stage:`,
          },
          {
            name: "LATER_SECRET",
            valueFrom: `${stack.resolve(secret.secretArn)}:field::`,
          },
        ],
      },
    ]);
    ...
    //     Match.objectLike({
    //       Secrets: [
    //         { Name: 'SECRET', ValueFrom: { Ref: 'SecretA720EF05' } },
    //         { Name: 'PARAMETER', ValueFrom: { 'Fn::Join': ['', ['arn:', { Ref: 'AWS::Partition' }, ':ssm:', { Ref: 'AWS::Region' }, ':', { Ref: 'AWS::AccountId' }, ':parameter/name']] } },
    //         { Name: 'SECRET_ID', ValueFrom: { 'Fn::Join': ['', [{ Ref: 'SecretA720EF05' }, ':::version-id']] } },
    //         { Name: 'SECRET_STAGE', ValueFrom: { 'Fn::Join': ['', [{ Ref: 'SecretA720EF05' }, '::version-stage:']] } },
    //         { Name: 'LATER_SECRET', ValueFrom: { 'Fn::Join': ['', [{ Ref: 'SecretA720EF05' }, ':field::']] } },
    //       ],
    //     }),
    //   ],
    // });
    ...
    //         Effect: 'Allow',
    //         Resource: { 'Fn::Join': ['', ['arn:', { Ref: 'AWS::Partition' }, ':ssm:', { Ref: 'AWS::Region' }, ':', { Ref: 'AWS::AccountId' }, ':parameter/name']] },
    //       },
    //     ],
    //     Version: '2012-10-17',
    //   },
  ...
  test("use a specific secret JSON key as environment variable", () => {
    ...
    expect(renderContainerDefinitions(stack)).toMatchObject([
      {
        secrets: [
          {
            name: "SECRET_KEY",
            valueFrom: `${stack.resolve(secret.secretArn)}:specificKey::`,
          },
        ],
      },
    ]);
    ...
    //     Match.objectLike({
    //       Secrets: [
    //         { Name: 'SECRET_KEY', ValueFrom: { 'Fn::Join': ['', [{ Ref: 'SecretA720EF05' }, ':specificKey::']] } },
    //       ],
    //     }),
    //   ],
    // });
  ...
  test("use a specific secret JSON field as environment variable for a Fargate task", () => {
    ...
    taskDefinition.addContainer("cont", {
      ...
      secrets: {
        SECRET_KEY: ecs.Secret.fromSecretsManager(secret, "specificKey"),
        SECRET_KEY_ID: ecs.Secret.fromSecretsManagerVersion(
          secret,
          { versionId: "version-id" },
          "specificKey",
        ),
    ...
    expect(renderContainerDefinitions(stack)).toMatchObject([
      {
        secrets: [
          ...
          {
            name: "SECRET_KEY_ID",
            valueFrom: `${stack.resolve(secret.secretArn)}:specificKey::version-id`,
          },
          {
            name: "SECRET_KEY_STAGE",
            valueFrom: `${stack.resolve(secret.secretArn)}:specificKey:version-stage:`,
          },
        ],
      },
    ]);
    ...
    //       Secrets: [
    //         { Name: 'SECRET_KEY', ValueFrom: { 'Fn::Join': ['', [{ Ref: 'SecretA720EF05' }, ':specificKey::']] } },
    //         { Name: 'SECRET_KEY_ID', ValueFrom: { 'Fn::Join': ['', [{ Ref: 'SecretA720EF05' }, ':specificKey::version-id']] } },
    //         { Name: 'SECRET_KEY_STAGE', ValueFrom: { 'Fn::Join': ['', [{ Ref: 'SecretA720EF05' }, ':specificKey:version-stage:']] } },
    //       ],
    //     }),
    //   ],
    // });
        ...
        statement: [
          {
            ...
            effect: "Allow",
            resources: [stack.resolve(logging.logGroup!.logGroupArn)],
          },
        ],
      },
    );
  ...
  test("throws when setting Health Check with no commands", () => {
    ...
    // THEN
    expect(() => renderContainerDefinitions(stack)).toThrow(
  ...
  test("throws when setting Health Check with invalid interval because of too short", () => {
    ...
    // THEN
    expect(() => renderContainerDefinitions(stack)).toThrow(
      /Interval must be between 5 seconds and 300 seconds./,
    );
  });
  ...
  test("throws when setting Health Check with invalid timeout because of too short", () => {
    ...
    // THEN
    expect(() => renderContainerDefinitions(stack)).toThrow(
      /Timeout must be between 2 seconds and 120 seconds./,
    );
  });
  ...
  test("throws when setting Health Check with invalid interval and timeout because timeout is longer than interval", () => {
    ...
    // THEN
    expect(() => renderContainerDefinitions(stack)).toThrow(
      /Health check interval should be longer than timeout./,
    );
  });
  ...
  test("can specify private registry credentials", () => {
    ...
    const taskDefinition = new ecs.Ec2TaskDefinition(stack, "TaskDef");
    const mySecretArn =
      "arn:aws:secretsmanager:region:1234567890:secret:MyRepoSecret-6f8hj3";

    const repoCreds = encryption.Secret.fromSecretCompleteArn(
      stack,
      "MyRepoSecret",
      mySecretArn,
    );
    ...
    //     Match.objectLike({
    //       Image: 'user-x/my-app',
    //       RepositoryCredentials: { CredentialsParameter: mySecretArn },
    //     }),
    //   ],
    // });
  ...
  describe("Can specify linux parameters", () => {
        ...
        expect(
          () =>
            new ecs.LinuxParameters(
              stack,
              `LinuxParametersWithSwappiness(${swappiness})`,
              { swappiness },
            ),
        ).toThrow(
          `swappiness: Must be an integer between 0 and 100; received ${swappiness}.`,
        ),
      );
    });
    ...
    test("after calling addContainer", () => {
      ...
      //       Image: 'test',
      //       LinuxParameters: {
      //         Capabilities: { Add: ['ALL'], Drop: ['SETUID'] },
      //         InitProcessEnabled: true,
      //         MaxSwap: 5 * 1024,
      //         SharedMemorySize: 1024,
      //         Swappiness: 90,
  ...
  test("can specify restart policy", () => {
    ...
    //       Name: 'Container',
    //       Memory: 2048,
    //       RestartPolicy: { Enabled: true, IgnoredExitCodes: [0, 127], RestartAttemptPeriod: 360 },
    //     },
    //   ],
    // });
  });
  ...
  test("restart policy will not be set if not specified", () => {
    ...
    // THEN
    expect(renderContainerDefinitions(stack)[0].restartPolicy).toBeUndefined();
    // Template.fromStack(stack).hasResourceProperties('AWS::ECS::TaskDefinition', {
    //   ContainerDefinitions: [
    //     {
    //       Image: '/aws/aws-example-app',
  ...
  test("enable restart policy when enableRestartPolicy is not specified but restartIgnoredExitCodes is specified", () => {
    ...
    //       Name: 'Container',
    //       Memory: 2048,
    //       RestartPolicy: { Enabled: true, IgnoredExitCodes: [0, 127] },
    //     },
    //   ],
    // });
  });
  ...
  test("enable restart policy when enableRestartPolicy is not specified but restartAttemptPeriod is specified", () => {
    ...
    //       Name: 'Container',
    //       Memory: 2048,
    //       RestartPolicy: { Enabled: true, RestartAttemptPeriod: 360 },
    //     },
    //   ],
    // });
  });
  ...
  test("throws when there are more than 50 in restartIgnoredExitCodes", () => {
    ...
    expect(() => {
      ...
      /Only up to 50 can be specified for restartIgnoredExitCodes, got: 51/,
    );
  });
  ...
  test("throws when restartAttemptPeriod is greater than 1800 seconds", () => {
    ...
    expect(() => {
      new ecs.ContainerDefinition(stack, "Container", {
        ...
        memoryLimitMiB: 2048,
        enableRestartPolicy: true,
        restartAttemptPeriod: Duration.seconds(1801),
      });
    }).toThrow(
      /The restartAttemptPeriod must be between 60 seconds and 1800 seconds, got 1801 seconds/,
    );
  });
  ...
  test("throws when restartAttemptPeriod is less than 60 seconds", () => {
    ...
    expect(() => {
      new ecs.ContainerDefinition(stack, "Container", {
        ...
        memoryLimitMiB: 2048,
        enableRestartPolicy: true,
        restartAttemptPeriod: Duration.seconds(59),
      });
    }).toThrow(
      /The restartAttemptPeriod must be between 60 seconds and 1800 seconds, got 59 seconds/,
    );
  });
  ...
  test("version consistency will not be set if not specified", () => {
    ...
    // THEN
    expect(
      renderContainerDefinitions(stack)[0].versionConsistency,
    ).toBeUndefined();
    // Template.fromStack(stack).hasResourceProperties('AWS::ECS::TaskDefinition', {
    //   ContainerDefinitions: [
    //     {
...
// Wrapping describe with synth/snapshot coverage for the ContainerDefinition
// construct exercised above (harness idiom: test/aws/notify/queue.test.ts +
// test/aws/compute/ecs/aws-log-driver.test.ts "aws log driver synth").
describe("container definition synth", () => {
  test("ContainerDefinition with a broad set of properties should synth and match SnapShot", () => {
    // GIVEN
 ...
 * Parse it back out so assertions can target individual containers the same way upstream's
 * `Match.objectLike` does against the CFN `ContainerDefinitions` array.
 */
