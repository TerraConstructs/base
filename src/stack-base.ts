...
export interface StackBaseProps {
  /**
   * TerraConstruct grid identifier used as a portable physical-name prefix.
   *
   * The gridUUID is incorporated into generated physical resource names (S3 bucket
   * prefixes, IAM role names, Lambda function names, etc.) to ensure uniqueness
   * across deployments. Because it appears at the start of physical names for many
   * AWS and Terraform resources, it **must begin with a letter** and contain only
   * alphanumeric characters and hyphens (`/^[A-Za-z][A-Za-z0-9-]*$/`).
   *
   * Note: digit-leading values (e.g. `123e4567-e89b-12d3`) are intentionally
   * rejected because they produce invalid physical names for services like IAM
   * and CloudWatch that require a leading alphabetic character.
   *
   * @default - Automatically generated from the construct path (prefixed with "g")
   */
    ...
    if (!VALID_GRID_UUID_REGEX.test(this.gridUUID)) {
      throw new ValidationError(
        `GridUUID must start with a letter and contain only alphanumeric characters or hyphens ` +
          `(pattern: ${VALID_GRID_UUID_REGEX.toString()}). Digit-leading values are rejected because ` +
          `gridUUID is used as a physical-name prefix for resources (IAM, CloudWatch, etc.) that ` +
          `require names to begin with a letter. Got '${this.gridUUID}'`,
        this,
      );
    }
