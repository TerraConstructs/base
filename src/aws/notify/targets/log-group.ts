...
// logGroup ResourcePolicy was added to CFN - https://github.com/aws/aws-cdk/pull/17015/files
 ...
 */
export class CloudWatchLogGroup implements IRuleTarget {
  ...
  public bind(_rule: IRule, _id?: string): RuleTargetConfig {
    ...
    if (this.target?.inputPath || this.target?.input) {
      throw new Error(
        "CloudWatchLogGroup targets does not support input or inputPath",
      );
    }
  ...
  private validateInputTemplate(): string[] {
    if (this.target?.inputTemplate) {
      ...
      if (typeof resolvedTemplate === "string") {
        ...
        const quotedTemplate = resolvedTemplate.replace(
          new RegExp("(<[^<>]*?>)", "g"),
          '"$1"',
        );
        try {
