...
class StepFunctionsExecutionIntegration extends AwsIntegration {
    ...
    super({
      ...
      options: {
        ...
        integrationResponses:
          options.integrationResponses ?? integrationResponse(),
  ...
  public bind(method: Method): IntegrationConfig {
    ...
    if (this.stateMachine instanceof sfn.StateMachine) {
      ...
      stateMachineName = (
        this.stateMachine.node.defaultChild as sfnStateMachine.SfnStateMachine
      ).nameInput;
    } else {
    ...
    if (
      ...
      !Token.isUnresolved(stateMachineName)
    ) {
...
function templateString(
  ...
  if (
    ...
    Object.keys(options.requestContext).length > 0
  ) {
    requestContextStr = requestContext(options.requestContext);
  }

  templateStr = fs.readFileSync(path.join(__dirname, "stepfunctions.vtl"), {
    encoding: "utf-8",
  });
...
function requestContext(requestContextObj: RequestContext | undefined): string {
  const context = {
    accountId: requestContextObj?.accountId
    ...
    cognitoAuthenticationProvider:
      requestContextObj?.cognitoAuthenticationProvider
    ...
    cognitoIdentityPoolId: requestContextObj?.cognitoIdentityPoolId
      ...
      : undefined,
    httpMethod: requestContextObj?.httpMethod
      ? "$context.httpMethod"
      : undefined,
    stage: requestContextObj?.stage ? "$context.stage" : undefined,
    sourceIp: requestContextObj?.sourceIp
      ? "$context.identity.sourceIp"
      : undefined,
    user: requestContextObj?.user ? "$context.identity.user" : undefined,
    userAgent: requestContextObj?.userAgent
      ? "$context.identity.userAgent"
      : undefined,
    userArn: requestContextObj?.userArn
      ? "$context.identity.userArn"
      : undefined,
    requestId: requestContextObj?.requestId ? "$context.requestId" : undefined,
    resourceId: requestContextObj?.resourceId
      ? "$context.resourceId"
      : undefined,
    resourcePath: requestContextObj?.resourcePath
