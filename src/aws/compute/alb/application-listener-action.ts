// https://github.com/aws/aws-cdk/blob/v2.175.1/packages/aws-cdk-lib/aws-elasticloadbalancingv2/lib/alb/application-listener-action.ts
...
export class ListenerAction implements IListenerAction {
   ...
   */
  public static authenticateOidc(
  ...
  public static forward(
    ...
    if (targetGroups.length === 0) {
      throw new Error(
        "Need at least one targetGroup in a ListenerAction.forward()",
      );
    }
  ...
  public static weightedForward(
    ...
    if (targetGroups.length === 0) {
      throw new Error(
        "Need at least one targetGroup in a ListenerAction.weightedForward()",
      );
    }
  ...
  public static redirect(options: RedirectOptions): ListenerAction {
    ...
    if (
      options.path &&
      !Token.isUnresolved(options.path) &&
      !options.path.startsWith("/")
    ) {
      throw new Error(
        `Redirect path must start with a \'/\', got: ${options.path}`,
      );
    }
   ...
   */
  protected addRuleAction(actionJson: tfListenerRule.LbListenerRuleAction) {
    if (this._actionJson) {
...
export interface AuthenticateOidcOptions {
   ...
   */
  readonly clientSecret: string; //SecretValue;
...
class AuthenticateOidcAction extends ListenerAction {
  ...
  constructor(options: AuthenticateOidcOptions) {
    const defaultActionConfig: tfListener.LbListenerDefaultActionAuthenticateOidc =
      {
        ...
        authenticationRequestExtraParams:
          options.authenticationRequestExtraParams,
