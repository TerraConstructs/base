...
export class ApplicationListenerRule extends Construct {
  ...
  constructor(
    ...
    const providedActions = possibleActions.filter(
      (action) => props[action] !== undefined,
    );
    if (providedActions.length > 1) {
      throw new Error(
        `'${providedActions}' specified together, specify only one`,
      );
    }

    if (!Token.isUnresolved(props.priority) && props.priority <= 0) {
      throw new Error("Priority must have value greater than or equal to 1");
    }

    this.listener = props.listener;

    const resource = new tfListenerRule.LbListenerRule(this, "Resource", {
    ...
    this.listenerRuleArn = resource.arn;
