...
export class NatGatewayProvider extends NatProvider {
  ...
  public configureNat(options: ConfigureNatOptions) {
    ...
    for (const sub of options.natSubnets) {
      ...
      this.gateways.add(sub.availabilityZone, gateway.id);
      i++;
    }
...
export class NatInstanceProvider extends NatProvider implements IConnectable {
  ...
  constructor(private readonly props: NatInstanceProps) {
    ...
    if (
      props.defaultAllowedTraffic !== undefined &&
      props.allowAllTraffic !== undefined
    ) {
      throw new Error(
  ...
  public configureNat(options: ConfigureNatOptions) {
    ...
    const role = new iam.Role(options.vpc, "NatRole", {
      assumedBy: new iam.ServicePrincipal("compute.amazonaws.com"),
    });
  ...
  public get securityGroup(): ISecurityGroup {
    if (!this._securityGroup) {
      throw new Error(
        "Pass the NatInstanceProvider to a Vpc before accessing 'securityGroup'",
      );
    }
  ...
  public get connections(): Connections {
    if (!this._connections) {
      throw new Error(
        "Pass the NatInstanceProvider to a Vpc before accessing 'connections'",
      );
    }
  ...
  public get configuredGateways(): GatewayConfig[] {
    return this.gateways
      .values()
      .map((x) => ({ az: x[0], gatewayId: x[1].instanceId }));
  }

  public configureSubnet(subnet: PrivateSubnet) {
    const az = subnet.availabilityZone;
    const gatewayInstance = this.gateways.pick(az);
    // TODO: Terraform provider does not support RouterType.INSTANCE
    // subnet.addRoute("DefaultRoute", {
