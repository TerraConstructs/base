// https://github.com/aws/aws-cdk/blob/v2.175.1/packages/@aws-cdk/aws-ec2-alpha/lib/subnet-v2.ts
 ...
 *
 */
export class SubnetV2 extends AwsConstructBase implements ISubnetV2 {
  /**
  ...
  public static fromSubnetV2Attributes(
     ...
     * @resource AWS::EC2::Subnet
     */
    class ImportedSubnetV2 extends AwsConstructBase implements ISubnetV2 {
      /**
      ...
      public subnetOutputs: SubnetOutputs = {
  ...
  public get subnetOutputs(): SubnetOutputs {
    return {
      subnetId: this.subnetId,
      ipv4CidrBlock: this.ipv4CidrBlock,
      routeTableId: this.routeTable.routeTableId,
  ...
  public resource: tfSubnet.Subnet;
  ...
  constructor(scope: Construct, id: string, props: SubnetV2Props) {
    ...
    overlap = validateOverlappingCidrRanges(
      props.vpc,
      props.ipv4CidrBlock.cidr,
    );

    //check whether VPC supports ipv6
    if (props.ipv6CidrBlock?.cidr) {
      validateSupportIpv6(props.vpc);
      overlapIpv6 = validateOverlappingCidrRangesipv6(
        props.vpc,
        props.ipv6CidrBlock?.cidr,
      );
    }

    if (overlap || overlapIpv6) {
      throw new Error(
        "CIDR block should not overlap with existing subnet blocks",
      );
    }

    if (props.assignIpv6AddressOnCreation && !props.ipv6CidrBlock) {
      throw new Error(
        "IPv6 CIDR block is required when assigning IPv6 address on creation",
      );
    }

    this.resource = new tfSubnet.Subnet(this, "Resource", {
    ...
    if (props.subnetName) {
      Tags.of(this).add(NAME_TAG, subnetName);
    }
    ...
    const routeAssoc = new tfRouteTableAssociation.RouteTableAssociation(
      this,
  ...
  public get networkAcl(): INetworkAcl | undefined {
    // TODO: should this rerturn from DataSource if undefined?
    // https://registry.terraform.io/providers/hashicorp/aws/5.68.0/docs/data-sources/network_acls
...
const subnetTypeMap = {
  ...
  [SubnetType.PUBLIC]: (vpc: IVpcV2, subnet: SubnetV2) =>
    vpc.publicSubnets.push(subnet),
  [SubnetType.PRIVATE_WITH_EGRESS]: (vpc: IVpcV2, subnet: SubnetV2) =>
    vpc.privateSubnets.push(subnet),
  [SubnetType.ISOLATED]: (vpc: IVpcV2, subnet: SubnetV2) =>
    vpc.isolatedSubnets.push(subnet),
  [SubnetType.PRIVATE]: (vpc: IVpcV2, subnet: SubnetV2) =>
    vpc.privateSubnets.push(subnet),
  [SubnetType.PRIVATE_WITH_NAT]: (vpc: IVpcV2, subnet: SubnetV2) =>
    vpc.privateSubnets.push(subnet),
};
 ...
 * @internal
 */
function storeSubnetToVpcByType(
  vpc: IVpcV2,
  subnet: SubnetV2,
  type: SubnetType,
) {
...
function checkCidrRanges(vpc: IVpcV2, cidrRange: string) {
  ...
  if (vpc.ipv4IpamProvisionedCidrs) {
    const cidrs = vpc.ipv4IpamProvisionedCidrs.map(
...
function validateOverlappingCidrRanges(
  vpc: IVpcV2,
 ...
 * @internal
 */
function validateOverlappingCidrRangesipv6(
  vpc: IVpcV2,
