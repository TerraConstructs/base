...
export class JsonPath {
  ...
  public static arrayLength(array: any): string {
    return new JsonPathToken(
      `States.ArrayLength(${renderInExpression(array)})`,
    ).toString();
  }
  ...
  public static arrayUnique(array: any): string {
    return new JsonPathToken(
      `States.ArrayUnique(${renderInExpression(array)})`,
    ).toString();
  }
  ...
  public static base64Encode(input: string): string {
    return new JsonPathToken(
      `States.Base64Encode(${renderInExpression(input)})`,
    ).toString();
  }
  ...
  public static base64Decode(base64: string): string {
    return new JsonPathToken(
      `States.Base64Decode(${renderInExpression(base64)})`,
    ).toString();
  }
  ...
  public static stringToJson(jsonString: string): IResolvable {
    return new JsonPathToken(
      `States.StringToJson(${renderInExpression(jsonString)})`,
    );
  }
  ...
  public static jsonToString(value: any): string {
    ...
    if (!path) {
      throw new Error(
        "Argument to JsonPath.jsonToString() must be a JsonPath object",
      );
    }
...
export class FieldUtils {
   ...
   */
  public static renderObject(obj?: {
    [key: string]: any;
  ...
  public static containsTaskToken(obj?: { [key: string]: any }): boolean {
    ...
    return (
      paths.has("$$.Task.Token") || paths.has("$$.Task") || paths.has("$$")
    );
  }
...
function validateJsonPath(path: string) {
  ...
  const intrinsicFunctionFullNames = intrinsicFunctionNames.map(
    (fn) => `States.${fn}`,
  );
  if (
    path !== "$" &&
    ...
    const intrinsicFunctionsStr =
      intrinsicFunctionFullNames.join(", ") + ", or " + lastItem;
    throw new Error(
