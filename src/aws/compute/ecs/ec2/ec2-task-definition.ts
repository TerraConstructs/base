// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-ecs/lib/ec2/ec2-task-definition.ts
 ...
 */
export class Ec2TaskDefinition
  ...
  implements IEc2TaskDefinition
{
  ...
  public static readonly PROPERTY_INJECTION_ID: string =
    "terraconstructs.aws.ecs.Ec2TaskDefinition";
    ...
    const invalidConstraints =
      constraints?.filter((constraint) => {
        return constraint
          ...
          .some(
            (constraintProperty) =>
              !validConstraints.has(constraintProperty.type),
          );
      }) ?? [];
    ...
    if (invalidConstraints.length > 0) {
      const invalidConstraintTypes = invalidConstraints
        .map((constraint) =>
          constraint
            .toJson()
            .map((constraintProperty) => constraintProperty.type),
        )
        .flat();
  ...
  constructor(
    ...
    Ec2TaskDefinition.validatePlacementConstraints(
      scope,
  ...
  override addContainer(
    ...
    if (this.networkMode === NetworkMode.AWS_VPC) {
      return super.addContainer(id, {
        ...
        environment: {
          ...props.environment,
          AWS_REGION: AwsStack.ofAwsConstruct(this).region,
        },
      });
    }
