// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-ecs/lib/ec2/ec2-service.ts
...
export class Ec2Service extends BaseService implements IEc2Service {
  ...
  public static readonly PROPERTY_INJECTION_ID: string =
    "terraconstructs.aws.ecs.Ec2Service";
  ...
  public static fromEc2ServiceArn(
    ...
    class Import extends AwsConstructBase implements IEc2Service {
    ...
    return new Import(scope, id, {
      environmentFromArn: ec2ServiceArn,
    });
  }
  ...
  private constraints?: PlacementConstraintConfig[];
  private strategies?: PlacementStrategyConfig[];
  ...
  constructor(scope: Construct, id: string, props: Ec2ServiceProps) {
    ...
    if (
      ...
      if (props.daemon) {
        throw new ValidationError(
          "AvailabilityZoneRebalancing.ENABLED cannot be used with daemon mode",
          scope,
        );
      }
      if (
    ...
    super(
      ...
      id,
      {
        ...
        maxHealthyPercent:
          props.daemon && props.maxHealthyPercent === undefined
            ? 100
            : props.maxHealthyPercent,
        minHealthyPercent:
          props.daemon && props.minHealthyPercent === undefined
            ? 0
      ...
      },
      {
        ...
        placementConstraints: Lazy.anyValue(
          { produce: () => this.constraints },
          { omitEmptyArray: true },
        ),
        orderedPlacementStrategy: Lazy.anyValue(
          { produce: () => this.strategies },
          { omitEmptyArray: true },
        ),
    ...
    if (props.taskDefinition.networkMode === NetworkMode.AWS_VPC) {
      ...
      this.connections.addSecurityGroup(
        ...securityGroupsInThisStack(
          this,
          props.cluster.connections.securityGroups,
        ),
      );
    }
    ...
    if (props.minHealthyPercent === undefined && props.daemon) {
      // TODO: Annotations.of(this).addWarningV2('@aws-cdk/aws-ecs:minHealthyPercentDaemon', ...)
  ...
  public addPlacementStrategies(...newStrategies: PlacementStrategy[]) {
    if (newStrategies.length > 0 && this.daemon) {
      throw new ValidationError(
        "Can't configure placement strategies when daemon=true",
        this,
      );
    }
    ...
    if (
      ...
      if (
        placement.type !== "spread" ||
        placement.field !== BuiltInAttributes.AVAILABILITY_ZONE
      ) {
        throw new ValidationError(
          `AvailabilityZoneBalancing.ENABLED requires that the first placement strategy, if any, be 'spread across "${BuiltInAttributes.AVAILABILITY_ZONE}"'`,
          this,
        );
      }
    }
    ...
    for (const constraint of constraints) {
      ...
      if (this.availabilityZoneRebalancingEnabled) {
        for (const item of items) {
          if (
            item.type === "memberOf" &&
            item.expression?.includes(BuiltInAttributes.AVAILABILITY_ZONE)
          ) {
            throw new ValidationError(
              `AvailabilityZoneBalancing.ENABLED disallows usage of "${BuiltInAttributes.AVAILABILITY_ZONE}"`,
              this,
            );
          }
        }
...
function securityGroupsInThisStack(
  ...
  const thisStack = AwsStack.ofAwsConstruct(scope);

  let i = 1;
  return groups.map((group) => {
    if (thisStack === AwsStack.ofAwsConstruct(group)) {
      return group;
