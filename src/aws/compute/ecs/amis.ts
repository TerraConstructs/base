// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-ecs/lib/amis.ts
 ...
 */
export class EcsOptimizedAmi implements IMachineImage {
  ...
  constructor(props?: EcsOptimizedAmiProps) {
    ...
    if (props && props.generation) {
      ...
      if (
        props.generation === AmazonLinuxGeneration.AMAZON_LINUX &&
      ...
      this.generation = AmazonLinuxGeneration.AMAZON_LINUX_2;
    }
    ...
    this.amiParameterName =
      ...
      (this.generation === AmazonLinuxGeneration.AMAZON_LINUX
        ? "amazon-linux/"
        : "") +
      (this.generation === AmazonLinuxGeneration.AMAZON_LINUX_2
        ? "amazon-linux-2/"
        : "") +
      (this.generation === AmazonLinuxGeneration.AMAZON_LINUX_2023
        ? "amazon-linux-2023/"
        : "") +
      (this.windowsVersion
    ...
    if (this.additionalCacheKey !== undefined && !this.cachedInContext) {
      throw new UnscopedValidationError(
        '"additionalCacheKey" was set but "cachedInContext" is false, so it will have no effect',
      );
    }
  }
  ...
  public getImage(scope: Construct): MachineImageConfig {
    const ami = lookupImage(
      scope,
      this.cachedInContext,
      this.amiParameterName,
      this.additionalCacheKey,
    );
 ...
 * Construct a Linux or Windows machine image from the latest ECS Optimized AMI published in SSM
 */
export class EcsOptimizedImage implements IMachineImage {
  /**
...
function lookupImage(
  ...
  _additionalCacheKey?: string,
) {
  // TERRACONSTRUCTS DEVIATION: upstream also supports StringParameter.valueFromLookup()
  // (cdk.context.json caching via ContextProvider) when cachedInContext is true. TerraConstructs
  // does not implement ContextProvider-based lookups (see repo convention on lookups). The
  // `_additionalCacheKey` parameter is retained (unused, upstream-parity signature) for when
  // ContextProvider-based caching is implemented -- it would fold into the cache key there.
  if (cachedInContext) {
    // TODO: ContextProvider-based cached lookup is not implemented in TerraConstructs.
    throw new UnscopedValidationError(
      "cachedInContext is not yet supported in TerraConstructs",
    );
  }
