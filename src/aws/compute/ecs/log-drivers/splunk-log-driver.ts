// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-ecs/lib/log-drivers/splunk-log-driver.ts
...
export interface SplunkLogDriverProps extends BaseLogDriverProps {
   ...
   *
   * // TERRACONSTRUCTS DEVIATION: upstream types this as `core.SecretValue`, which is not ported
   * to terraconstructs yet (same gap noted in `compute/vpn.ts` and
   * `compute/alb/application-listener-action.ts`). Pass a plain string; it is used verbatim, so it
   * will end up resolved and viewable in plain text just like the upstream `unsafeUnwrap()` usage.
   * @deprecated Use `SplunkLogDriverProps.secretToken` instead.
   * @default - token not provided.
   */
  readonly token?: string; //SecretValue;
