// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-ecs/lib/log-drivers/aws-log-driver.ts
...
import { ILogGroup, LogGroup } from "../../../cloudwatch";
  ...
  constructor(private readonly props: AwsLogDriverProps) {
    ...
    if (props.logGroup && props.logRetention) {
      throw new UnscopedValidationError(
        "Cannot specify both `logGroup` and `logRetentionDays`.",
      );
    }

    if (props.maxBufferSize && props.mode !== AwsLogDriverMode.NON_BLOCKING) {
      throw new UnscopedValidationError(
        "Cannot specify `maxBufferSize` when the driver mode is blocking",
      );
    }
  }
