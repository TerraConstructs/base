// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-ecs/lib/external/external-service.ts
...
export class ExternalService extends BaseService implements IExternalService {
  ...
  public static readonly PROPERTY_INJECTION_ID: string =
    "terraconstructs.aws.ecs.ExternalService";
  ...
  public static fromExternalServiceArn(
    ...
    return new Import(scope, id, {
      environmentFromArn: externalServiceArn,
    });
  }
  ...
  constructor(scope: Construct, id: string, props: ExternalServiceProps) {
    if (props.daemon) {
      ...
      if (props.desiredCount !== undefined) {
        throw new ValidationError(
          "Daemon mode launches one task on every instance. Cannot specify desiredCount when daemon mode is enabled.",
          scope,
        );
      }
      if (
        ...
        throw new ValidationError(
          "Maximum percent must be 100 when daemon mode is enabled.",
          scope,
        );
      }
    }
    ...
    if (props.cloudMapOptions !== undefined) {
      throw new ValidationError(
        "Cloud map options are not supported for External service",
        scope,
      );
    }
    ...
    if (props.capacityProviderStrategies !== undefined) {
      throw new ValidationError(
        "Capacity Providers are not supported for External service",
        scope,
      );
    }

    const propagateTagsFromSource =
      props.propagateTags ?? PropagatedTagSource.NONE;
    ...
    super(
      ...
      id,
      {
        ...
        maxHealthyPercent:
          props.maxHealthyPercent === undefined ? 100 : props.maxHealthyPercent,
        minHealthyPercent:
          props.minHealthyPercent === undefined ? 0 : props.minHealthyPercent,
  ...
  public loadBalancerTarget(
    ...
    throw new ValidationError(
      "External service cannot be attached as load balancer targets",
      this,
    );
  }
  ...
  public registerLoadBalancerTargets(..._targets: EcsTarget[]) {
    throw new ValidationError(
      "External service cannot be registered as load balancer targets",
      this,
    );
  }
  ...
  protected configureAwsVpcNetworkingWithSecurityGroups(
    ...
    throw new ValidationError(
      "Only Bridge network mode is supported for external service",
      this,
    );
  }
  ...
  public autoScaleTaskCount(_props: EnableScalingProps): ScalableTaskCount {
    throw new ValidationError(
      "Autoscaling not supported for external service",
      this,
    );
  }
   ...
   */
  public enableCloudMap(_options: CloudMapOptions): edge.cloudmap.Service {
    throw new ValidationError(
