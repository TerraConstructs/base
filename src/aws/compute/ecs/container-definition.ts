// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-ecs/lib/container-definition.ts
...
import { ISecret } from "../../encryption/secret";
import * as iam from "../../iam";
import { IParameter } from "../../storage/parameter";
...
export interface ContainerDefinitionOptions {
   ...
   */
  readonly restartAttemptPeriod?: Duration;
}
...
export class ContainerDefinition extends Construct {
  ...
  public static readonly PROPERTY_INJECTION_ID: string =
    "terraconstructs.aws.ecs.ContainerDefinition";
  ...
  constructor(
    ...
    if (
      ...
      props.memoryReservationMiB !== undefined
    ) {
    ...
    if (props.inferenceAcceleratorResources) {
      this.addInferenceAcceleratorResource(
        ...props.inferenceAcceleratorResources,
      );
    }
  ...
  public addLink(container: ContainerDefinition, alias?: string) {
    if (this.taskDefinition.networkMode !== NetworkMode.BRIDGE) {
      throw new ValidationError(
        "You must use network mode Bridge to add container links.",
        this,
      );
    }
    if (alias !== undefined) {
  ...
  public addPortMappings(...portMappings: PortMapping[]) {
    this.portMappings.push(
      ...portMappings.map((pm) => {
        ...
        portMap.validate();
        const serviceConnect = new ServiceConnect(
          this.taskDefinition.networkMode,
          pm,
        );
   ...
   */
  public addContainerDependencies(
  ...
  private setNamedPort(pm: PortMapping): void {
    ...
    if (this._namedPorts.has(pm.name)) {
      throw new ValidationError(
        `Port mapping name '${pm.name}' already exists on this container`,
        this,
      );
    }
  ...
  private validateRestartPolicy(
    ...
    if (
      ...
      (restartAttemptPeriod.toSeconds() < 60 ||
        restartAttemptPeriod.toSeconds() > 1800)
    ) {
      throw new ValidationError(
  ...
  public get ingressPort(): number {
    ...
    if (
      defaultPortMapping.hostPort !== undefined &&
      defaultPortMapping.hostPort !== 0
    ) {
  ...
  /**
   * Render this container definition to the plain container-definition JSON object embedded
   * in the task definition's jsonencoded `container_definitions`.
   *
   * // TERRACONSTRUCTS DEVIATION: upstream renders to `CfnTaskDefinition.ContainerDefinitionProperty`
   * (a CloudFormation L1 property type sourced from `ecs.generated.ts`). TerraConstructs has no CFN
   * L1 layer here -- the `aws_ecs_task_definition` resource takes `container_definitions` as a single
   * jsonencode()'d string built from an array of these plain `ContainerDefinitionConfig` objects, so
   * this renders directly to that shape instead.
   *
  ...
  public renderContainerDefinition(
    ...
    return {
      ...
      credentialSpecs:
        this.credentialSpecs && this.credentialSpecs.map(renderCredentialSpec),
      cpu: this.props.cpu,
      disableNetworking: this.props.disableNetworking,
      dependsOn: Lazy.anyValue(
        {
          produce: () =>
            this.containerDependencies.map(renderContainerDependency),
        },
        { omitEmptyArray: true },
      ),
      dnsSearchDomains: this.props.dnsSearchDomains,
      dnsServers: this.props.dnsServers,
      dockerLabels: Object.keys(this.dockerLabels).length
        ? this.dockerLabels
        : undefined,
      ...
      startTimeout:
        this.props.startTimeout && this.props.startTimeout.toSeconds(),
      ...
      environmentFiles:
        ...
        renderEnvironmentFiles(
          AwsStack.ofAwsConstruct(this).partition,
          this.environmentFiles,
        ),
      secrets: this.secrets.length ? this.secrets : undefined,
      extraHosts:
        this.props.extraHosts &&
        renderKV(this.props.extraHosts, "hostname", "ipAddress"),
      healthCheck:
        this.props.healthCheck &&
        renderHealthCheck(this, this.props.healthCheck),
      links: Lazy.listValue({ produce: () => this.links }, { omitEmpty: true }),
      linuxParameters:
      ...
      systemControls:
        this.props.systemControls &&
        renderSystemControls(this.props.systemControls),
...
/**
 * The plain container-definition JSON object embedded in the task definition's jsonencoded
 * `container_definitions`. Replaces upstream's `CfnTaskDefinition.ContainerDefinitionProperty`
 * (see the `// TERRACONSTRUCTS DEVIATION` note on `renderContainerDefinition`).
 */
export interface ContainerDefinitionConfig {
  ...
  readonly cpu?: number;
  readonly disableNetworking?: boolean;
  readonly dependsOn?: ContainerDependencyConfig[] | IResolvable;
  readonly dnsSearchDomains?: string[];
  readonly dnsServers?: string[];
  readonly dockerLabels?: { [key: string]: string };
  readonly dockerSecurityOptions?: string[];
  ...
  readonly portMappings?: PortMappingConfig[] | IResolvable;
  readonly privileged?: boolean;
  readonly pseudoTerminal?: boolean;
  readonly readonlyRootFilesystem?: boolean;
  readonly repositoryCredentials?: ContainerImageConfig["repositoryCredentials"];
  readonly startTimeout?: number;
  readonly stopTimeout?: number;
  readonly ulimits?: UlimitConfig[] | IResolvable;
  readonly user?: string;
  readonly versionConsistency?: VersionConsistency;
  readonly volumesFrom?: VolumeFromConfig[] | IResolvable;
  readonly workingDirectory?: string;
  readonly logConfiguration?: LogDriverConfig;
  readonly environment?: KeyValuePairConfig[];
  readonly environmentFiles?: EnvironmentFileEntryConfig[];
  readonly secrets?: SecretConfig[];
  readonly extraHosts?: HostEntryConfig[];
  readonly healthCheck?: HealthCheckConfig;
  readonly links?: string[];
  readonly linuxParameters?: LinuxParametersConfig;
  readonly resourceRequirements?: ResourceRequirementConfig[];
  readonly systemControls?: SystemControlConfig[];
  readonly restartPolicy?: RestartPolicyConfig;
}

/**
 * A `name`/`value` pair, as embedded in `ContainerDefinitionConfig.environment`. Replaces
 * upstream's `CfnTaskDefinition.KeyValuePairProperty`.
 */
...
/**
 * A `hostname`/`ipAddress` pair, as embedded in `ContainerDefinitionConfig.extraHosts`. Replaces
 * upstream's `CfnTaskDefinition.HostEntryProperty`.
 */
...
/**
 * An environment file entry, as embedded in `ContainerDefinitionConfig.environmentFiles`.
 * Replaces upstream's `CfnTaskDefinition.EnvironmentFileProperty`.
 */
export interface EnvironmentFileEntryConfig {
  readonly type: EnvironmentFileType;
  readonly value: string;
}
...
/**
 * The rendered health check, as embedded in `ContainerDefinitionConfig.healthCheck`. Replaces
 * upstream's `CfnTaskDefinition.HealthCheckProperty`.
 */
...
function renderHealthCheck(
  ...
  if (
    hc.interval?.toSeconds() !== undefined &&
    hc.timeout?.toSeconds() !== undefined
  ) {
...
function renderResourceRequirements(
  gpuCount: number = 0,
  inferenceAcceleratorResources: string[] = [],
): ResourceRequirementConfig[] | undefined {
  const ret = [];
...
/**
 * A resource requirement entry, as embedded in `ContainerDefinitionConfig.resourceRequirements`.
 * Replaces upstream's `CfnTaskDefinition.ResourceRequirementProperty`.
 */
...
/**
 * The rendered ulimit, as embedded in `ContainerDefinitionConfig.ulimits`. Replaces upstream's
...
function renderUlimit(ulimit: Ulimit): UlimitConfig {
  return {
    name: ulimit.name,
...
/**
 * The rendered container dependency, as embedded in `ContainerDefinitionConfig.dependsOn`.
 * Replaces upstream's `CfnTaskDefinition.ContainerDependencyProperty`.
 */
...
function renderContainerDependency(
  ...
  return {
    ...
    condition:
      containerDependency.condition || ContainerDependencyCondition.HEALTHY,
  };
}
  ...
  public validate(): void {
    if (!this.isvalidPortName()) {
      throw new UnscopedValidationError(
        "Port mapping name cannot be an empty string.",
      );
    }

    if (
      this.portmapping.containerPort ===
    ...
    if (
      ...
      if (
        (this.networkmode === NetworkMode.AWS_VPC ||
    ...
    if (this.portmapping.containerPortRange !== undefined) {
      if (Token.isUnresolved(this.portmapping.containerPortRange)) {
        throw new UnscopedValidationError(
      ...
      if (
        this.networkmode !== NetworkMode.BRIDGE &&
        this.networkmode !== NetworkMode.AWS_VPC
      ) {
        throw new UnscopedValidationError(
...
/**
 * The rendered port mapping, as embedded in `ContainerDefinitionConfig.portMappings`. Replaces
 * upstream's `CfnTaskDefinition.PortMappingProperty`.
 */
...
function renderPortMapping(pm: PortMapping): PortMappingConfig {
  return {
    containerPort:
...
/**
 * The rendered mount point, as embedded in `ContainerDefinitionConfig.mountPoints`. Replaces
 * upstream's `CfnTaskDefinition.MountPointProperty`.
 */
...
function renderMountPoint(mp: MountPoint): MountPointConfig {
  return {
...
/**
 * The rendered volume-from entry, as embedded in `ContainerDefinitionConfig.volumesFrom`.
 * Replaces upstream's `CfnTaskDefinition.VolumeFromProperty`.
 */
export interface VolumeFromConfig {
  readonly sourceContainer: string;
  readonly readOnly: boolean;
}

function renderVolumeFrom(vf: VolumeFrom): VolumeFromConfig {
  return {
...
/**
 * The rendered system control entry, as embedded in `ContainerDefinitionConfig.systemControls`.
 * Replaces upstream's `CfnTaskDefinition.SystemControlProperty`.
 */
...
/**
 * The rendered secret entry, as embedded in `ContainerDefinitionConfig.secrets`. Replaces
 * upstream's `CfnTaskDefinition.SecretProperty`.
 */
...
/**
 * The rendered restart policy, as embedded in `ContainerDefinitionConfig.restartPolicy`. Replaces
 * upstream's `CfnTaskDefinition.RestartPolicyProperty`.
 */
