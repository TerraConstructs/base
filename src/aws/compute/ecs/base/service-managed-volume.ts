// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-ecs/lib/base/service-managed-volume.ts
...
export class ServiceManagedVolume extends Construct {
  ...
  private validateEbsVolumeConfiguration(
    ...
    if (
      volumeInitializationRate !== undefined &&
      ...
      if (
        volumeInitializationRate.toMebibytes() < 100 ||
        volumeInitializationRate.toMebibytes() > 300
      ) {
        throw new ValidationError(
    ...
    if (
      snapShotId &&
      !Token.isUnresolved(snapShotId) &&
      !/^snap-[0-9a-fA-F]+$/.test(snapShotId)
    ) {
      throw new ValidationError(
    ...
    if (throughput !== undefined) {
      if (volumeType !== ec2.EbsDeviceVolumeType.GP3) {
        ...
        throw new ValidationError(
          `'throughput' must be less than or equal to 2000 MiB/s, got ${throughput} MiB/s`,
          this,
        );
      }
    }
    ...
    if (
      [ec2.EbsDeviceVolumeType.IO1, ec2.EbsDeviceVolumeType.IO2].includes(
        volumeType,
      ) &&
      iops === undefined
    ) {
      throw new ValidationError(
        `'iops' must be specified with io1 or io2 volume types, got ${volumeType}`,
        this,
      );
    }
