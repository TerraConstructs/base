// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-ecs/lib/base/base-service.ts

import { ecsService } from "@cdktn/provider-aws";
import { Annotations, Fn, Lazy, Token } from "cdktn";
...
import { ApplicationProtocol, TargetType } from "../../lb-shared/enums";
import { LoadBalancer, ILoadBalancerTarget } from "../../load-balancer";
import {
  AddNetworkTargetsProps,
  NetworkListener,
} from "../../nlb/network-listener";
import {
  INetworkLoadBalancerTarget,
  INetworkTargetGroup,
  NetworkTargetGroup,
} from "../../nlb/network-target-group";
import { ServiceNamespace } from "../../scalable-target";
...
export interface IEcsLoadBalancerTarget
  extends IApplicationLoadBalancerTarget,
    INetworkLoadBalancerTarget,
    ILoadBalancerTarget {}
 ...
 */
export interface BaseServiceOptions extends AwsConstructProps {
  /**
...
class ApplicationListenerConfig extends ListenerConfig {
  constructor(
    ...
    private readonly props?: AddApplicationTargetsProps,
  ) {
    super();
  }
  ...
  public addTargets(
    ...
    const port =
      props.port ?? (protocol === ApplicationProtocol.HTTPS ? 443 : 80);
...
class NetworkListenerConfig extends ListenerConfig {
  constructor(
    ...
    private readonly props?: AddNetworkTargetsProps,
  ) {
    super();
  }
    ...
    ILoadBalancerTarget
{
  ...
  public static fromServiceArnWithCluster(
    ...
    if (Token.isUnresolved(resourceName)) {
      clusterName = Fn.element(Fn.split("/", resourceName), 0);
      serviceName = Fn.element(Fn.split("/", resourceName), 1);
    } else {
    ...
    const clusterArn = stack.formatArn({
  ...
  /**
   * The underlying `aws_ecs_service` Terraform resource.
   */
  public readonly resource: ecsService.EcsService;

  /**
   * The details of the AWS Cloud Map service.
   */
  protected cloudmapService?: edge.cloudmap.Service;

  /**
   * The load balancer target descriptions -- rendered directly into the `load_balancer` blocks of
   * the underlying `aws_ecs_service` resource.
   */
  protected loadBalancers = new Array<ecsService.EcsServiceLoadBalancer>();

  /**
   * The network configuration for the service -- rendered directly into the `network_configuration`
   * block of the underlying `aws_ecs_service` resource.
   */
  protected networkConfiguration?: ecsService.EcsServiceNetworkConfiguration;

  /**
   * The deployment alarms property - this will be rendered directly and lazily as the `alarms`
   * block of the underlying `aws_ecs_service` resource.
   */
  protected deploymentAlarms?: ecsService.EcsServiceAlarms;

  /**
   * TERRACONSTRUCTS DEVIATION: upstream models this as an array (`serviceRegistries`) of at most
   * one CFN `ServiceRegistryProperty`, because `AWS::ECS::Service.ServiceRegistries` is typed as a
   * list even though the service enforces at most one entry. The `aws_ecs_service` Terraform
   * resource types `service_registries` as a single nested block instead of a list, so this is
   * kept as a singular optional field (still enforced to be set at most once, see
   * `addServiceRegistry()`).
   */
  protected serviceRegistry?: ecsService.EcsServiceServiceRegistries;

  /**
   * The service connect configuration for this service.
   * @internal
   */
  protected _serviceConnectConfig?: ecsService.EcsServiceServiceConnectConfiguration;
  ...
  /**
   * The deployment strategy to use for the service, as passed to the constructor. Held onto so
   * that `toTerraform()` can rebuild the `deployment_configuration` block (merged with any
   * lifecycle hooks added after construction) without needing the original `props`.
   */
  private readonly deploymentStrategy?: DeploymentStrategy;

  /**
   * The stringified bake time in minutes, as passed to the constructor. See
   * `deploymentStrategy` above for why this is held onto as an instance field.
   */
  private readonly bakeTimeInMinutes?: string;

  /**
   * Constructs a new instance of the BaseService class.
   *
   * @param additionalProps TERRACONSTRUCTS DEVIATION: upstream types this `any` because it is
   * spread verbatim into the underlying `CfnServiceProps` object literal (a subclass-supplied
   * fragment of `cluster`/`taskDefinition`/placement fields). This still spreads verbatim into the
   * `aws_ecs_service` Terraform resource's `EcsServiceConfig`, so subclasses (`Ec2Service`,
   * `FargateService`) must supply fields already shaped like `EcsServiceConfig`
   * (e.g. `cluster: cluster.clusterName`, `taskDefinition: taskDefinition.taskDefinitionArn`) rather
   * than the upstream `CfnServiceProps` shape.
   */
  constructor(
    scope: Construct,
    id: string,
    ...
    // launchType will be undefined if using external DeploymentController or capacityProviderStrategies
    const launchType =
    ...
    const propagateTagsFromSource =
      props.propagateTaskTagsFrom ??
      props.propagateTags ??
      PropagatedTagSource.NONE;

    // TERRACONSTRUCTS DEVIATION: upstream calls a `getDeploymentController()` helper that also sets
    // an explicit `{ type: DeploymentControllerType.ECS }` here when `circuitBreaker` is configured,
    // unless the `@aws-cdk/aws-ecs:disableExplicitDeploymentControllerForCircuitBreaker` feature flag
    // is enabled (`recommendedValue: true`). This repo always targets the modern/recommended CDK
    // behavior (see `cluster.ts`, `alb/application-listener.ts` for the same convention), so that
    // legacy branch is dropped -- the deployment controller defaults to ECS naturally (see
    // `isEcsDeploymentController` below) when left unset.
    const deploymentController = props.deploymentController;

    // Determine if this service is using the ECS deployment controller
    this.isEcsDeploymentController =
      !deploymentController ||
      deploymentController.type === DeploymentControllerType.ECS;

    // HARD REPO INVARIANT 1 (naming): `aws_ecs_service` supports `name` only (no `name_prefix`),
    // so this falls back to the stack-scoped exact-form unique name rather than upstream's
    // CloudFormation-generated-name default.
    const serviceName =
      props.serviceName ?? this.stack.uniqueResourceName(this);

    this.deploymentStrategy = props.deploymentStrategy;
    this.bakeTimeInMinutes = props.bakeTime?.toMinutes().toString();

    // TERRACONSTRUCTS DEVIATION: `alarms`, `deploymentConfiguration`, `networkConfiguration`,
    // `serviceRegistries`, `serviceConnectConfiguration`, and `volumeConfiguration` are NOT set
    // here. Upstream lazily resolves these onto CFN's `CfnService` L1 via `Lazy.any({ produce })`
    // (CloudFormation accepts an `IResolvable` token for any property). The `aws_ecs_service`
    // Terraform resource types each of these as a single nested config BLOCK rather than a token
    // -- the generated `EcsService` L1 does not accept `IResolvable` for them, only the literal
    // shape -- so the generated class instead exposes dedicated `putXxx()` setters. Values that
    // accumulate via imperative APIs called after construction (`enableDeploymentAlarms()`,
    // `configureAwsVpcNetworking(WithSecurityGroups)()`, `enableCloudMap()`/
    // `associateCloudMapService()`, `enableServiceConnect()`, `addVolume()`, `addLifecycleHook()`)
    ...
    this.resource = new ecsService.EcsService(this, "Resource", {
      ...
      // `Lazy.anyValue()` bypasses cdktn's normal `listMapper(ecsServiceLoadBalancerToTerraform)`
      // attribute conversion, since it runs before the Lazy value resolves. Mapping through
      // `ecsServiceLoadBalancerToTerraform` here ensures snake_case terraform attribute names
      // (e.g. `container_name`, `elb_name`) are emitted instead of the raw camelCase keys.
      loadBalancer: Lazy.anyValue(
        {
          produce: () =>
            this.loadBalancers.map(
              ecsService.ecsServiceLoadBalancerToTerraform,
            ),
        },
        { omitEmptyArray: true },
      ),
      deploymentMaximumPercent: props.maxHealthyPercent || 200,
      deploymentMinimumHealthyPercent:
        props.minHealthyPercent === undefined ? 50 : props.minHealthyPercent,
      ...
      healthCheckGracePeriodSeconds: this.evaluateHealthGracePeriod(
        props.healthCheckGracePeriod,
      ),
      /* iamRole: never specified, supplanted by Service Linked Role */
      ...additionalProps,
    });
    ...
    if (props.deploymentAlarms && !this.isEcsDeploymentController) {
      throw new ValidationError(
        "Deployment alarms requires the ECS deployment controller.",
        this,
      );
    }

    if (
      props.deploymentController?.type ===
    ...
    if (
      ...
      // NOTE: must not use `this.resource.taskDefinition += ...` here -- reading
      // `this.resource.taskDefinition` back off the generated L1 invokes its
      // getter, which returns the `aws_ecs_service` resource's own `task_definition`
      // *attribute* token (e.g. `${aws_ecs_service.<id>.task_definition}`), not the
      // literal value just assigned via the setter. `+=` would therefore make the
      // resource self-referentially depend on its own attribute.
      this.resource.taskDefinition =
        props.taskDefinitionRevision !== TaskDefinitionRevision.LATEST
          ? `${taskDefinition.family}:${props.taskDefinitionRevision.revision}`
          : taskDefinition.family;
      this.node.addDependency(taskDefinition);
    }

    this.serviceArn = this.resource.arn;
    this.serviceName = this.resource.name;
    ...
    if (props.enableExecuteCommand) {
      ...
      const logging =
        this.cluster.executeCommandConfiguration?.logging ??
    ...
    // TERRACONSTRUCTS DEVIATION: upstream also sets a default (disabled, empty-alarm-list)
    // `deploymentAlarms` block here when the service uses the ECS deployment controller and the
    // `@aws-cdk/aws-ecs:removeDefaultDeploymentAlarm` feature flag is NOT enabled. That flag's
    // `recommendedValue` is `true` (i.e. "do not set a default"), and this repo always targets the
    // modern/recommended CDK behavior, so the legacy default-alarms branch -- and the
    // `deploymentAlarmsAvailableInRegion()` partition check it depended on -- is dropped entirely.
  ...
  private renderLifecycleHooks(): ecsService.EcsServiceDeploymentConfigurationLifecycleHook[] {
  ...
  private renderDeploymentConfiguration():
    | ecsService.EcsServiceDeploymentConfiguration
    | undefined {
    const lifecycleHook = this.renderLifecycleHooks();

    if (
      this.deploymentStrategy === undefined &&
      this.bakeTimeInMinutes === undefined &&
      lifecycleHook.length === 0
    ) {
      return undefined;
    }
    ...
    return {
      ...
      bakeTimeInMinutes: this.bakeTimeInMinutes,
      lifecycleHook: lifecycleHook.length > 0 ? lifecycleHook : undefined,
    };
  }
   ...
   *
   * TERRACONSTRUCTS DEVIATION: mirrors the `lb-shared/base-target-group.ts` `toTerraform()`
   * idiom. See the comment above the `EcsService` construction in the constructor for why these
   * fields cannot be set inline via `Lazy.anyValue()`. `putXxx()` is idempotent (it simply
   * overwrites the resource's internal value), so it is safe to call unconditionally on every
   * `prepareStack()` pass.
   */
  public toTerraform(): any {
    if (this.deploymentAlarms) {
      this.resource.putAlarms(this.deploymentAlarms);
    }

    const deploymentConfiguration = this.renderDeploymentConfiguration();
    if (deploymentConfiguration) {
      this.resource.putDeploymentConfiguration(deploymentConfiguration);
    }

    if (this.networkConfiguration) {
      this.resource.putNetworkConfiguration(this.networkConfiguration);
    }

    if (this.serviceRegistry) {
      this.resource.putServiceRegistries(this.serviceRegistry);
    }

    if (this._serviceConnectConfig) {
      this.resource.putServiceConnectConfiguration(this._serviceConnectConfig);
    }

    const volumeConfiguration = this.renderVolumeConfiguration();
    if (volumeConfiguration) {
      this.resource.putVolumeConfiguration(volumeConfiguration);
    }
  ...
  private renderVolumeConfiguration():
    ...
    return renderVolume(this.volumes[0], this);

    function renderVolume(
      spec: ServiceManagedVolume,
      scope: BaseService,
    ): ecsService.EcsServiceVolumeConfiguration {
      // TERRACONSTRUCTS DEVIATION: the `aws_ecs_service` Terraform resource's
      // `volume_configuration.managed_ebs_volume` block is REQUIRED (unlike CFN's
      // `ServiceVolumeConfigurationProperty.managedEBSVolume`, which is technically optional).
      // `ServiceManagedVolume` always populates `config` (defaulting an auto-created role even when
      // no `managedEBSVolume` props were supplied), so this only throws in the theoretical case
      // where a future upstream change leaves `config` undefined.
      if (!spec.config) {
        throw new ValidationError(
          `ServiceManagedVolume '${spec.name}' has no managed EBS volume configuration; the 'aws_ecs_service' Terraform resource requires 'managed_ebs_volume' to be set.`,
          scope,
        );
      }

      const tagSpecifications = spec.config.tagSpecifications?.map(
        (
          ebsTagSpec,
      ...
      return {
        ...
        managedEbsVolume: {
          ...
          sizeInGb: spec.config.size?.toGibibytes(),
          volumeInitializationRate:
            spec.config.volumeInitializationRate?.toMebibytes(),
          tagSpecifications,
        },
      };
    }
  ...
  public enableDeploymentAlarms(
    ...
    if (this.deploymentAlarms?.enable && options?.behavior) {
      if (
        (AlarmBehavior.ROLLBACK_ON_ALARM === options.behavior &&
          !this.deploymentAlarms.rollback) ||
        (AlarmBehavior.FAIL_ON_ALARM === options.behavior &&
          this.deploymentAlarms.rollback)
      ) {
        throw new ValidationError(
  ...
  public enableServiceConnect(config?: ServiceConnectProps) {
    ...
    /**
     * Map services to the `service_connect_configuration.service` blocks. This block manages:
     * 1. Finding the correct port.
     * 2. Client alias enumeration
     */
    const services = cfg.services?.map(
      (svc): ecsService.EcsServiceServiceConnectConfigurationService => {
        const containerPort = this.taskDefinition.findPortMappingByName(
          svc.portMappingName,
        )?.containerPort;
        if (!containerPort) {
          throw new ValidationError(
            `Port mapping with name ${svc.portMappingName} does not exist.`,
            this,
          );
        }
        const alias: ecsService.EcsServiceServiceConnectConfigurationServiceClientAlias =
          {
    ...
    this._serviceConnectConfig = {
      ...
      logConfiguration: logConfig
        ? {
            ...
            secretOption: logConfig.secretOptions,
          }
        : undefined,
      namespace: namespace,
      service: services,
  ...
  private validateServiceConnectConfiguration(config?: ServiceConnectProps) {
    ...
    config.services.forEach((serviceConnectService) => {
      ...
      if (
        !this.taskDefinition.findPortMappingByName(
          serviceConnectService.portMappingName,
        )
      ) {
        throw new ValidationError(
      ...
      if (
        portNames
          .get(serviceConnectService.portMappingName)
          ?.includes(discoveryName)
      ) {
        throw new ValidationError(
      ...
      let currentDiscoveries = portNames.get(
        serviceConnectService.portMappingName,
      );
      ...
      if (
        serviceConnectService.port &&
        !this.isValidPort(serviceConnectService.port)
      ) {
        throw new ValidationError(
          `Client Alias port ${serviceConnectService.port} is not valid.`,
          this,
        );
      }
      ...
      if (
        ...
        !awsPcaAuthorityArn.startsWith("arn:")
      ) {
        throw new ValidationError(
          `awsPcaAuthorityArn must start with "arn:" and have at least 6 components; received ${awsPcaAuthorityArn}`,
          this,
        );
      }
    });
   ...
   */
  public get cloudMapService(): edge.cloudmap.IService | undefined {
    return this.cloudmapService;
  }

  private executeCommandLogConfiguration() {
    // TERRACONSTRUCTS DEVIATION: upstream gates this on the
    // `@aws-cdk/aws-ecs:reduceEc2FargateCloudWatchPermissions` feature flag (recommendedValue:
    // true) to avoid granting a wildcard-resource CloudWatch Logs policy when no CloudWatch log
    // group is configured. This repo always targets the modern/recommended CDK behavior, so the
    // broad-permission branch for the "no log group configured" case is dropped -- these
    // permissions are only granted when `logConfiguration.cloudWatchLogGroup` is set.
    const logConfiguration =
      this.cluster.executeCommandConfiguration?.logConfiguration;

    if (logConfiguration?.cloudWatchLogGroup) {
      ...
      const logGroupArn = `arn:${this.stack.partition}:logs:${this.env.region}:${this.env.account}:log-group:${logConfiguration.cloudWatchLogGroup.logGroupName}:*`;
      this.taskDefinition.addToTaskRolePolicy(
        new iam.PolicyStatement({
          actions: [
    ...
    if (logConfiguration?.s3Bucket?.bucketName) {
      ...
      this.taskDefinition.addToTaskRolePolicy(
        new iam.PolicyStatement({
          ...
          resources: [
            `arn:${this.stack.partition}:s3:::${logConfiguration.s3Bucket.bucketName}/*`,
          ],
        }),
      );
  ...
  private enableExecuteCommandEncryption(logging: ExecuteCommandLogging) {
    this.taskDefinition.addToTaskRolePolicy(
      new iam.PolicyStatement({
        ...
        resources: [
          `${this.cluster.executeCommandConfiguration?.kmsKey?.keyArn}`,
        ],
      }),
    );
      ...
      new iam.PolicyStatement({
        ...
        principals: [
          new iam.ArnPrincipal(
            `arn:${this.stack.partition}:iam::${this.env.account}:root`,
          ),
        ],
      }),
    );
    ...
    if (
      ...
      this.cluster.executeCommandConfiguration?.kmsKey?.addToResourcePolicy(
        new iam.PolicyStatement({
          ...
          resources: ["*"],
          // NOTE: pass the bare service name (not a `logs.<region>.amazonaws.com`
          // string built via `this.env.region`) so `ServicePrincipal` resolves the
          // regional principal through the `aws_service_principal` data source lazily.
          // `this.env.region` is frequently an unresolved token (e.g. the
          // `data.aws_region.current.name` fallback used when the stack has no
          // explicit region), and embedding an unresolved token into the *service*
          // string breaks `AwsStack.servicePrincipalName()`'s construct-id generation
          // (construct ids cannot contain tokens).
          principals: [new iam.ServicePrincipal("logs")],
          condition: [
            {
              test: "ArnLike",
              variable: "kms:EncryptionContext:aws:logs:arn",
              values: [
                `arn:${this.stack.partition}:logs:${this.env.region}:${this.env.account}:*`,
              ],
            },
          ],
        }),
  ...
  public attachToApplicationTargetGroup(
    ...
    return this.defaultLoadBalancerTarget.attachToApplicationTargetGroup(
      targetGroup,
    );
  }
  ...
  public attachToNetworkTargetGroup(
    ...
    return this.defaultLoadBalancerTarget.attachToNetworkTargetGroup(
      targetGroup,
    );
  }
   ...
   */
  public autoScaleTaskCount(props: EnableScalingProps) {
    if (this.scalableTaskCount) {
      throw new ValidationError(
        "AutoScaling of task count already enabled for this service",
        this,
      );
    }
   ...
   */
  public enableCloudMap(options: CloudMapOptions): edge.cloudmap.Service {
    const sdNamespace =
      options.cloudMapNamespace ?? this.cluster.defaultCloudMapNamespace;
    ...
    if (sdNamespace.type === edge.cloudmap.NamespaceType.HTTP) {
      throw new ValidationError(
    ...
    if (
      ...
      if (dnsRecordType !== edge.cloudmap.DnsRecordType.SRV) {
        throw new ValidationError(
    ...
    const cloudmapService = new edge.cloudmap.Service(this, "CloudmapService", {
   ...
   */
  public associateCloudMapService(
  ...
  protected configureAwsVpcNetworking(
    ...
    if (securityGroup === undefined) {
      securityGroup = new SecurityGroup(this, "SecurityGroup", { vpc });
    }
    ...
    this.networkConfiguration = {
      ...
      securityGroups: Lazy.listValue({
        produce: () => [securityGroup!.securityGroupId],
      }),
    };
  }
  ...
  private addServiceRegistry(registry: ServiceRegistry) {
    if (this.serviceRegistry !== undefined) {
      throw new ValidationError(
    ...
    this.serviceRegistry = this.renderServiceRegistry(registry);
  }
  ...
  private attachToELBv2(
    ...
    containerName: string,
    containerPort: number,
    alternateTarget?: IAlternateTarget,
  ): LoadBalancerTargetProps {
    if (this.taskDefinition.networkMode === NetworkMode.NONE) {
      throw new ValidationError(
  ...
  private evaluateHealthGracePeriod(
    providedHealthCheckGracePeriod?: Duration,
  ): number {
...
export interface CloudMapOptions {
   ...
   */
  readonly cloudMapNamespace?: edge.cloudmap.INamespace;
   ...
   */
  readonly dnsRecordType?:
    | edge.cloudmap.DnsRecordType.A
    | edge.cloudmap.DnsRecordType.SRV;
...
export interface AssociateCloudMapServiceOptions {
   ...
   */
  readonly service: edge.cloudmap.IService;
...
interface DetermineContainerNameAndPortOptions {
  dnsRecordType: edge.cloudmap.DnsRecordType;
  taskDefinition: TaskDefinition;
 ...
 */
function determineContainerNameAndPort(
  scope: IConstruct,
  options: DetermineContainerNameAndPortOptions,
) {
  ...
  if (options.dnsRecordType === edge.cloudmap.DnsRecordType.SRV) {
    // Ensure the user-provided container is from the right task definition.
    if (
      options.container &&
      options.container.taskDefinition != options.taskDefinition
    ) {
      throw new ValidationError(
        "Cannot add discovery for a container from another task definition",
        scope,
      );
    }

    const container =
      options.container ?? options.taskDefinition.defaultContainer!;

    // Ensure that any port given by the user is mapped.
    if (
      options.containerPort &&
      !container.portMappings.some(
        (mapping) => mapping.containerPort === options.containerPort,
      )
    ) {
      throw new ValidationError(
        "Cannot add discovery for a container port that has not been mapped",
        scope,
      );
    }
    ...
    return {
      ...
      containerPort:
        options.containerPort ??
        options.taskDefinition.defaultContainer!.containerPort,
    };
  }
