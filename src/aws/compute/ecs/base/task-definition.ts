// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-ecs/lib/base/task-definition.ts
 ...
 */
export interface ITaskDefinition extends IAwsConstruct {
  /**
 ...
 */
export interface CommonTaskDefinitionProps extends AwsConstructProps {
  /**
...
/**
 * Not exported upstream either -- purely internal to organize the shared
 * `isXxxCompatible` getters and the `outputs` projection for `TaskDefinition`.
 */
abstract class TaskDefinitionBase
...
export class TaskDefinition extends TaskDefinitionBase {
  ...
  public static readonly PROPERTY_INJECTION_ID: string =
    "terraconstructs.aws.ecs.TaskDefinition";
  ...
  /**
   * The underlying `aws_ecs_task_definition` Terraform resource.
   */
  public readonly resource: ecsTaskDefinition.EcsTaskDefinition;
  ...
  private readonly placementConstraints =
    new Array<ecsTaskDefinition.EcsTaskDefinitionPlacementConstraints>();

  /**
   * Inference accelerators for task instances
   *
   * TERRACONSTRUCTS DEVIATION: the terraform-provider-aws `aws_ecs_task_definition` resource
   * has no `inference_accelerators` argument (confirmed absent from the provider schema), so
   * there is no Terraform field to write these values to. The public `inferenceAccelerators`
   * getter, `addInferenceAccelerator()` method, this accumulator, and the associated
   * compatibility validations are preserved for interface-compatibility with upstream and
   * because `ContainerDefinition.addInferenceAcceleratorResource()` validates against this
   * list -- but the values are otherwise inert (this feature is deprecated upstream, EOL
   * since April 2024).
   */
  ...
  constructor(scope: Construct, id: string, props: TaskDefinitionProps) {
    ...
    this.family = props.family ?? this.stack.uniqueResourceName(this);
    ...
    if (this.isFargateCompatible && this.networkMode !== NetworkMode.AWS_VPC) {
      throw new ValidationError(
        `Fargate tasks can only have AwsVpc network mode, got: ${this.networkMode}`,
        this,
      );
    }
    ...
    if (
      ...
      ![NetworkMode.BRIDGE, NetworkMode.HOST, NetworkMode.NONE].includes(
        this.networkMode,
      )
    ) {
      throw new ValidationError(
    ...
    if (this.isManagedInstancesCompatible) {
      ...
      if (![NetworkMode.AWS_VPC, NetworkMode.HOST].includes(this.networkMode)) {
        throw new ValidationError(
          `Managed Instances tasks can only have AwsVpc or Host network mode, got: ${this.networkMode}`,
          this,
        );
      }
      ...
      if (
        ...
        throw new ValidationError(
          "Cannot use inference accelerators on tasks that run on Managed Instances",
          this,
        );
      }
      ...
      if (props.ephemeralStorageGiB) {
        throw new ValidationError(
          "Ephemeral storage is not supported for tasks running on Managed Instances",
          this,
        );
      }
      ...
      if (props.ipcMode) {
        throw new ValidationError(
          "IPC mode is not supported for tasks running on Managed Instances",
          this,
        );
      }
      ...
      if (props.proxyConfiguration) {
        throw new ValidationError(
          "Proxy configuration is not supported for tasks running on Managed Instances",
          this,
        );
      }
      ...
      if (
        props.runtimePlatform?.operatingSystemFamily &&
        !props.runtimePlatform.operatingSystemFamily.isLinux()
      ) {
        throw new ValidationError(
          `Managed Instances tasks only support LINUX operating system family, got: ${props.runtimePlatform.operatingSystemFamily._operatingSystemFamily}`,
          this,
        );
      }
    }
    ...
    if (
      ...
      ![NetworkMode.AWS_VPC, NetworkMode.HOST].includes(this.networkMode)
    ) {
      throw new ValidationError(
        `Only AWS_VPC and HOST Network Modes are supported for enabling Fault Injection, got ${this.networkMode} mode.`,
        this,
      );
    }
    ...
    if (props.inferenceAccelerators) {
      props.inferenceAccelerators.forEach((ia) =>
        this.addInferenceAccelerator(ia),
      );
    }
    ...
    // TERRACONSTRUCTS DEVIATION: upstream's `containerDefinitions`/`volumes`/`placementConstraints`
    // are typed Cfn L1 array properties (each individually Lazy-rendered, CFN handles the JSON
    // encoding). The `aws_ecs_task_definition` Terraform resource instead models
    // `container_definitions` as a SINGLE jsonencode()'d string, so the whole containers array is
    // rendered and stringified together. `volume` and `placement_constraints` remain typed nested
    // TF blocks and keep the same Lazy-array shape as upstream.
    //
    // NOTE: must go through `this.stack.toJsonString()` (Tokenization.resolve() + JSON.stringify),
    // NOT a bare `JSON.stringify()` -- `renderContainers()` embeds nested `Lazy.anyValue()`/
    // `Lazy.listValue()` tokens (mountPoints, portMappings, dependsOn, ulimits, volumesFrom, links,
    // etc.) whose `toJSON()` returns the literal placeholder string `"<unresolved-lazy>"`; only
    // `Tokenization.resolve()` (scope- and preparing-aware) correctly resolves those before the
    // object graph is stringified. See `notify/rule.ts` `eventPattern` for the same pattern.
    const taskDefProps: ecsTaskDefinition.EcsTaskDefinitionConfig = {
      containerDefinitions: Lazy.stringValue({
        produce: () => this.stack.toJsonString(this.renderContainers()),
      }),
      // NOTE: `renderVolumes()` returns camelCase config objects (matching the
      // `EcsTaskDefinitionVolume` TS interface). Because this is wrapped in
      // `Lazy.anyValue()`, cdktn's normal `listMapper(ecsTaskDefinitionVolumeToTerraform)`
      // attribute conversion (which runs at `synthesizeAttributes()` time, before the
      // Lazy resolves) never sees a real array to map -- it just passes the unresolved
      // token through untouched. Applying `ecsTaskDefinitionVolumeToTerraform` here,
      // inside the producer, ensures the emitted keys are the terraform snake_case
      // attribute names (e.g. `host_path`) instead of the raw camelCase config keys.
      volume: Lazy.anyValue(
        {
          produce: () =>
            this.renderVolumes().map(
              ecsTaskDefinition.ecsTaskDefinitionVolumeToTerraform,
            ),
        },
        { omitEmptyArray: true },
      ),
      executionRoleArn: Lazy.stringValue({
      ...
      requiresCompatibilities: [
        ...
        ...(isManagedInstancesCompatible(props.compatibility)
          ? ["MANAGED_INSTANCES"]
          : []),
      ],
      ...
      placementConstraints: Lazy.anyValue(
        {
          produce: () =>
            ...
            !isManagedInstancesCompatible(this.compatibility)
    ...
    const taskDef = new ecsTaskDefinition.EcsTaskDefinition(
      this,
      "Resource",
      taskDefProps,
    );
    this.resource = taskDef;

    if (props.placementConstraints) {
      props.placementConstraints.forEach((pc) =>
        this.addPlacementConstraint(pc),
      );
    }
  ...
  private renderVolumes(): ecsTaskDefinition.EcsTaskDefinitionVolume[] {
    ...
    function renderVolume(
      ...
      return {
        hostPath: spec.host?.sourcePath,
        name: spec.name,
        configureAtLaunch: spec.configuredAtLaunch,
        ...
        efsVolumeConfiguration: spec.efsVolumeConfiguration && {
          ...
          transitEncryptionPort:
            spec.efsVolumeConfiguration.transitEncryptionPort,
        },
      };
    }
  }
  ...
  public _validateTarget(
    ...
    const targetContainerPort =
      options.containerPort || targetContainer.containerPort;
    const portMapping = targetContainer.findPortMapping(
      targetContainerPort,
      targetProtocol,
    );
   ...
   * @internal
   */
  public _portRangeFromPortMapping(portMapping: PortMapping): Port {
    if (portMapping.hostPort !== undefined && portMapping.hostPort !== 0) {
      return portMapping.protocol === Protocol.UDP
        ? Port.udp(portMapping.hostPort)
        : Port.tcp(portMapping.hostPort);
    }
    if (
      this.networkMode === NetworkMode.BRIDGE ||
      this.networkMode === NetworkMode.NAT
    ) {
      return EPHEMERAL_PORT_RANGE;
    }
    if (
      portMapping.containerPort !== ContainerDefinition.CONTAINER_PORT_USE_RANGE
    ) {
      return portMapping.protocol === Protocol.UDP
        ? Port.udp(portMapping.containerPort)
    ...
    const [startPort, endPort] = portMapping
      ...
      .map((v) => Number(v));
    return portMapping.protocol === Protocol.UDP
      ? Port.udpRange(startPort, endPort)
      : Port.tcpRange(startPort, endPort);
  }
  ...
  public addFirelensLogRouter(
    ...
    if (this.containers.find((x) => x instanceof FirelensLogRouter)) {
      throw new ValidationError(
        "Firelens log router is already added in this task.",
        this,
      );
    }
  ...
  private validateTaskDefinition(): string[] {
    ...
    if (runtimeVolumes.length > 1) {
      ...
      ret.push(
        `More than one volume is configured at launch: [${volumeNames}]`,
      );
    }
    ...
    for (const volume of this.volumes) {
      if (volume.configuredAtLaunch) {
        const isVolumeMounted = this.containers.some((container) => {
          return container.mountPoints.some(
            (mp) => mp.sourceVolume === volume.name,
          );
        });
 ...
 */
export function isManagedInstancesCompatible(
