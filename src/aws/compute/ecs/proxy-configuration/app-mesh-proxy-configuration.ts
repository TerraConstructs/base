// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-ecs/lib/proxy-configuration/app-mesh-proxy-configuration.ts
...
export class AppMeshProxyConfiguration extends ProxyConfiguration {
  ...
  public bind(
    ...
    return {
      ...
      properties: renderProperties(configProps),
      type: configType,
    };
  }
}

// TERRACONSTRUCTS DEVIATION: upstream renders `proxyConfigurationProperties` as an
// Array<KeyValuePairProperty> ({name,value} pairs) because CfnTaskDefinition.ProxyConfigurationProperty
// models it that way. The `aws_ecs_task_definition` `proxy_configuration.properties` argument is a
// `{[key: string]: string}` map instead, so the key/value pairs are built directly into a map here
// rather than an array of name/value objects.
function renderProperties(props: AppMeshProxyConfigurationProps): {
  [key: string]: string;
} {
