 ...
 * @see https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-ec2-ipampool.html
 */
interface IpamPoolProps extends PoolOptions, AwsConstructProps {
  /**
   ...
   * changed after they have allocated space or if the allocation tagging requirements are changed on
   * the pool, the resource may be marked as noncompliant.
   */
  readonly allocationResourceTags?: Record<string, string>;

  // TODO: Add support for more arguments
  // @see https://registry.terraform.io/providers/hashicorp/aws/5.68.0/docs/resources/vpc_ipam_pool#argument-reference
  // @see https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-ec2-ipampool.html
}
 ...
 * @internal
 */
class IpamPool extends AwsConstructBase implements IIpamPool {
  public get ipamPoolOutputs(): IpamPoolOutputs {
    return {
   ...
   * Pool CIDR for IPv6 to be provisioned with Public IP source set to 'Amazon'
   */
  public readonly ipamCidrs: tfIpamPoolCidr.VpcIpamPoolCidr[] = [];
   ...
   */
  private readonly _ipamPool: tfIpamPool.VpcIpamPool;
  ...
  constructor(scope: Construct, id: string, props: IpamPoolProps) {
    ...
    if (props.addressFamily === AddressFamily.IP_V6 && !props.awsService) {
      throw new Error(
        "awsService is required when addressFamily is set to ipv6",
      );
    }

    //Add tags to the IPAM Pool if name is provided
    if (ipamPoolName) {
      Tags.of(this).add(NAME_TAG, ipamPoolName);
    }

    this._ipamPool = new tfIpamPool.VpcIpamPool(this, "Resource", {
 ...
 */
class IpamScope extends AwsConstructBase implements IIpamScopeBase {
  public get ipamScopeOutputs(): IpamScopeOutputs {
    return {
   ...
   */
  private readonly _ipamScope: tfIpamScope.VpcIpamScope;
  ...
  constructor(scope: Construct, id: string, props: IpamScopeProps) {
    super(scope, id);
    this._ipamScope = new tfIpamScope.VpcIpamScope(scope, "Resource", {
      ipamId: props.ipamId,
    });
    Tags.of(this._ipamScope).add(
      NAME_TAG,
      props.ipamScopeName ?? "CustomIpamScope",
    );
 ...
 */
export class Ipam extends AwsConstructBase {
  public get ipamOutputs(): IpamOutputs {
    return {
      ipamId: this.ipamId,
    };
  }
  ...
  // Resource IPAM
  private readonly _ipam: tfIpam.VpcIpam;
  /**
  ...
  constructor(scope: Construct, id: string, props?: IpamProps) {
    ...
    this._ipam = new tfIpam.VpcIpam(this, "Ipam", {
...
function createIpamPool(
  ...
  if (!isLocaleInOperatingRegions) {
    throw new Error(
      `The provided locale '${poolOptions.locale}' is not in the operating regions.`,
    );
  }
