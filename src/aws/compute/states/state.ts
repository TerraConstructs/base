...
export abstract class State extends Construct implements IChainable {
   ...
   */
  public static findReachableStates(
    start: State,
   ...
   */
  public static findReachableEndStates(
    start: State,
    options: FindStateOptions = {},
  ): State[] {
    const visited = new Set<State>();
  ...
  public get stateId(): string {
    return this.prefixes
      .concat(this.stateName ? this.stateName : this.id)
      .join("");
  }
  ...
  public bindToGraph(graph: StateGraph) {
    ...
    for (const outgoing of this.outgoingTransitions({
  ...
  protected renderRetryCatch(): any {
    return {
      Retry: renderList(this.retries, renderRetry, (a, b) =>
        compareErrors(a.errors, b.errors),
      ),
