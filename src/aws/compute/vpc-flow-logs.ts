...
export abstract class FlowLogDestination {
   ...
   */
  public static toKinesisDataFirehoseDestination(
   ...
   */
  public abstract bind(scope: Construct, f: FlowLog): FlowLogDestinationConfig;
}
...
class S3Destination extends FlowLogDestination {
  ...
  public bind(scope: Construct, _flowLog: FlowLog): FlowLogDestinationConfig {
    let s3Bucket: storage.IBucket;
    if (this.props.s3Bucket === undefined) {
      s3Bucket = new storage.Bucket(scope, "Bucket", {
        // TODO: Ensure flowlog Bucket is retained on delete?
    ...
    const prefix = this.props.destinationOptions?.hiveCompatiblePartitions
      ? s3Bucket.arnForObjects(
          `${keyPrefix}AWSLogs/aws-account-id=${stack.account}/*`,
        )
      ...
      destinationOptions:
        ...
        this.props.destinationOptions?.hiveCompatiblePartitions
          ? {
              ...
              perHourPartition:
                this.props.destinationOptions.perHourPartition ?? false,
  ...
  public bind(scope: Construct, _flowLog: FlowLog): FlowLogDestinationConfig {
    ...
    if (this.props.iamRole === undefined) {
      iamRole = new iam.Role(scope, "IAMRole", {
        roleName: AwsStack.uniqueId(scope), //PhysicalName.GENERATE_IF_NEEDED,
...
export class LogFormat {
   ...
   */
  public static readonly PKT_SRC_AWS_SERVICE = LogFormat.field(
    "pkt-src-aws-service",
  );
   ...
   */
  public static readonly PKT_DST_AWS_SERVICE = LogFormat.field(
    "pkt-dst-aws-service",
  );
   ...
   */
  public static readonly ECS_CONTAINER_INSTANCE_ARN = LogFormat.field(
   ...
   */
  public static readonly ECS_CONTAINER_INSTANCE_ID = LogFormat.field(
   ...
   */
  public static readonly ECS_SECOND_CONTAINER_ID = LogFormat.field(
   ...
   */
  public static readonly ECS_TASK_DEFINITION_ARN = LogFormat.field(
 ...
 */
abstract class FlowLogBase extends AwsConstructBase implements IFlowLog {
  /**
   * The Id of the VPC Flow Log
   *
   * @attribute
  ...
  public get flowLogOutputs(): FlowLogOutputs {
    return {
      flowLogId: this.flowLogId,
    };
  }
  public get outputs(): Record<string, any> {
    return this.flowLogOutputs;
  }
}
...
export class FlowLog extends FlowLogBase {
   ...
   */
  public readonly bucket?: storage.IBucket;
  ...
  public readonly resource: flowLog.FlowLog;

  constructor(scope: Construct, id: string, props: FlowLogProps) {
    super(scope, id);

    const destination =
      props.destination || FlowLogDestination.toCloudWatchLogs();
    ...
    let trafficType: FlowLogTrafficType | undefined =
      props.trafficType ?? FlowLogTrafficType.ALL;
    if (
    ...
    // convert AWSCC -> AWS resourceIdProps
    // TODO: Adopt AWSCC
    let resourceIdProps: {
      eniId?: string;
      subnetId?: string;
    ...
    switch (props.resourceType.resourceType) {
      case "NetworkInterface":
        resourceIdProps = { eniId: props.resourceType.resourceId };
        break;
      case "Subnet":
        resourceIdProps = { subnetId: props.resourceType.resourceId };
        break;
      case "TransitGateway":
        resourceIdProps = { transitGatewayId: props.resourceType.resourceId };
        break;
      case "TransitGatewayAttachment":
        resourceIdProps = {
          transitGatewayAttachmentId: props.resourceType.resourceId,
        };
        break;
      case "VPC":
        resourceIdProps = { vpcId: props.resourceType.resourceId };
        break;
      default:
        throw new Error("Invalid resource type: " + props.resourceType);
    }

    this.resource = new flowLog.FlowLog(this, "Resource", {
    ...
    if (this.bucket?.policy?.node.defaultChild instanceof TerraformResource) {
      this.resource.node.addDependency(this.bucket?.policy.node.defaultChild);
    }

    // TODO: we must remove a flow log configuration first before deleting objects.
    // TODO: S3 Bucket object removal custom resource?
    // const deleteObjects = this.bucket?.node.tryFindChild(
    //   "AutoDeleteObjectsCustomResource",
    // )?.node.defaultChild;
    // if (deleteObjects instanceof TerraformResource) {
    //   resource.node.addDependency(deleteObjects);
    // }

    this.flowLogId = this.resource.id;
  }
}
