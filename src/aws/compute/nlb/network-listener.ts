...
import { validateNetworkProtocol } from "../lb-shared/util";
...
export class NetworkListener extends BaseListener implements INetworkListener {
  ...
  //         break;
  //       case Protocol.UDP:
  //         listenerProtocol = LoadBalancerListenerProtocol.UDP;
  //         break;
  //       case Protocol.TCP_UDP:
  //         listenerProtocol = LoadBalancerListenerProtocol.TCP_UDP;
  //         break;
  //       case Protocol.TLS:
  //         listenerProtocol = LoadBalancerListenerProtocol.TLS;
  //         break;
  //     }
  //   }
  ...
  //   class LookedUp extends AwsConstructBase implements INetworkListener {
  ...
  public static fromNetworkListenerArn(
    ...
    class Import extends AwsConstructBase implements INetworkListener {
      public listenerArn = networkListenerArn;
      public listenerOutputs = {
        listenerArn: networkListenerArn,
      };
      public outputs = {
        listenerArn: networkListenerArn,
      };
   ...
   */
  private readonly protocol: LbProtocol;

  constructor(scope: Construct, id: string, props: NetworkListenerProps) {
    const certs = props.certificates || [];
    const proto =
      props.protocol || (certs.length > 0 ? LbProtocol.TLS : LbProtocol.TCP);

    validateNetworkProtocol(proto);

    if (
      proto === LbProtocol.TLS &&
      certs.filter((v) => v != null).length === 0
    ) {
      throw new Error(
    ...
    if (proto !== LbProtocol.TLS && certs.length > 0) {
      throw new Error(
        "Protocol must be TLS when certificates have been specified",
      );
    }

    if (proto !== LbProtocol.TLS && props.alpnPolicy) {
      throw new Error(
        "Protocol must be TLS when alpnPolicy have been specified",
      );
    }
    ...
    if (props.defaultTargetGroups) {
      this.setDefaultAction(
        NetworkListenerAction.forward(props.defaultTargetGroups),
      );
    }

    if (
      props.tcpIdleTimeout !== undefined &&
      !Token.isUnresolved(props.tcpIdleTimeout)
    ) {
      if (
        props.tcpIdleTimeout.toMilliseconds() <
        Duration.seconds(1).toMilliseconds()
      ) {
        throw new Error(
          `\`tcpIdleTimeout\` must be between 60 and 6000 seconds, got ${props.tcpIdleTimeout.toMilliseconds()} milliseconds.`,
        );
      }

      const tcpIdleTimeoutSeconds = props.tcpIdleTimeout.toSeconds();

      if (proto === LbProtocol.UDP) {
        throw new Error(
      ...
      this.setAttribute(
        ...
        tcpIdleTimeoutSeconds.toString(),
      );
    }
  }
