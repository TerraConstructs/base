 ...
 */
function orderAndCompleteIntervals(
  intervals: ScalingInterval[],
): CompleteScalingInterval[] {
...
function combineUndefineds(intervals: CompleteScalingInterval[]) {
  ...
  while (i < intervals.length - 1) {
    if (
      intervals[i].change === undefined &&
      intervals[i + 1].change === undefined
    ) {
...
function propagateBounds(intervals: ScalingInterval[]) {
  ...
  for (let i = 0; i < intervals.length - 1; i++) {
    if (
      intervals[i].upper !== undefined &&
      intervals[i + 1].lower === undefined
    ) {
  ...
  for (let i = intervals.length - 1; i >= 1; i--) {
    if (
      intervals[i].lower !== undefined &&
      intervals[i - 1].upper === undefined
    ) {
 ...
 */
export function findAlarmThresholds(
  ...
  if (gapIndex !== -1) {
    return {
      ...
      upperAlarmIntervalIndex:
        gapIndex < intervals.length - 1 ? gapIndex + 1 : undefined,
    };
  }
