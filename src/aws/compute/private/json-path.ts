...
export function jsonPathFromAny(x: any) {
  ...
  const resolvables = Tokenization.reverse(x);
  if (resolvables.length === 1) {
    return pathFromToken(resolvables[0]);
  }
  if (resolvables.length > 1) {
    // TODO: Should never happen, but if it does, we should throw a better error
    throw new Error(
      `Unexpected value: ${x} (expected a string or a single resolvable)`,
    );
  }
  return undefined;
}
...
function pathFromToken(token: IResolvable | undefined) {
  return (
    token && (JsonPathToken.isJsonPathToken(token) ? token.path : undefined)
  );
}
