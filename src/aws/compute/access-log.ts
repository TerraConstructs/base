...
import { ILogGroup } from "../cloudwatch";
...
export class FirehoseLogDestination implements IAccessLogDestination {
  constructor(
    private readonly stream: kinesisFirehoseDeliveryStream.KinesisFirehoseDeliveryStream,
  ) {}
  ...
  public bind(_stage: IStage): AccessLogDestinationConfig {
    if (!this.stream.nameInput?.startsWith("amazon-apigateway-")) {
      ...
      throw new Error(
        `Firehose delivery stream name for access log destination must begin with 'amazon-apigateway-', got '${this.stream.nameInput}'`,
        // _stage,
      );
    }
    return {
...
export class AccessLogFormat {
  ...
  public static clf(): AccessLogFormat {
    ...
    return new AccessLogFormat(
      `${requester} [${requestTime}] "${request}" ${status}`,
    );
  }
