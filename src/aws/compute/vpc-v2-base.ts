// https://github.com/aws/aws-cdk/blob/v2.175.1/packages/@aws-cdk/aws-ec2-alpha/lib/vpc-v2-base.ts
...
import { FlowLogOptions, FlowLog, FlowLogResourceType } from "./vpc-flow-logs";
import {
...
import { AccountPrincipal, Effect, PolicyStatement, Role } from "../iam";
 ...
 */
export abstract class VpcV2Base extends AwsConstructBase implements IVpcV2 {
  /**
   ...
   */
  public addEgressOnlyInternetGateway(
    options?: EgressOnlyInternetGatewayOptions,
  ): void {
    ...
    if (options?.subnets) {
      const subnets = flatten(
        options.subnets.map((s) => this.selectSubnets(s).subnets),
      );
  ...
  public addInternetGateway(options?: InternetGatewayOptions): void {
    ...
    if (this.publicSubnets) {
      this.publicSubnets.forEach((s) =>
        this.addDefaultInternetRoute(s, igw, options),
      );
    }
  }
  ...
  private addDefaultInternetRoute(
    ...
    if (subnet.subnetType !== SubnetType.PUBLIC) {
      throw new Error(
        "No public subnets defined to add route for internet gateway",
      );
    }
  ...
  public addNatGateway(options: NatGatewayOptions): NatGateway {
    if (
      options.connectivityType === NatConnectivityType.PUBLIC &&
      !this._internetGatewayId
    ) {
      throw new Error(
  ...
  public createAcceptorVpcRole(requestorAccountId: string): Role {
    ...
    peeringRole.addToPolicy(
      new PolicyStatement({
        ...
        resources: [
          `arn:${AwsStack.ofAwsConstruct(this).partition}:ec2:${this.region}:${this.ownerAccountId}:vpc/${this.vpcId}`,
        ],
      }),
    );
    ...
    peeringRole.addToPolicy(
      new PolicyStatement({
        ...
        effect: Effect.ALLOW,
        resources: [
          `arn:${AwsStack.ofAwsConstruct(this).partition}:ec2:${this.region}:${this.ownerAccountId}:vpc-peering-connection/*`,
        ],
        condition: [
          {
            test: "StringEquals",
