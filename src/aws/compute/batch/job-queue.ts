// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-batch/lib/job-queue.ts
 ...
 */
export interface IJobQueue extends IAwsConstruct {
  /**
 ...
 */
export interface JobQueueProps extends AwsConstructProps {
  /**
   * The set of compute environments mapped to a job queue and their order relative to each other.
   * The job scheduler uses this parameter to determine which compute environment runs a specific job.
   * Compute environments must be in the VALID state before you can associate them with a job queue.
 ...
 */
export class JobQueue extends AwsConstructBase implements IJobQueue {
  /** Uniquely identifies this class. */
  public static readonly PROPERTY_INJECTION_ID: string =
    "terraconstructs.aws.batch.JobQueue";

  /**
   * refer to an existing JobQueue by its arn
   */
  public static fromJobQueueArn(
    scope: Construct,
    id: string,
    jobQueueArn: string,
  ): IJobQueue {
    const stack = AwsStack.ofAwsConstruct(scope);
    class Import extends AwsConstructBase implements IJobQueue {
      public readonly computeEnvironments: OrderedComputeEnvironment[] = [];
      public readonly priority = 1;
      public readonly jobQueueArn = jobQueueArn;
      public readonly jobQueueName = stack.splitArn(
        jobQueueArn,
      ...
      public get outputs(): Record<string, any> {
        return {
          ...
          name: this.jobQueueName,
        };
      }

      public addComputeEnvironment(
        _computeEnvironment: IComputeEnvironment,
        _order: number,
      ): void {
        throw new ValidationError(
          `cannot add ComputeEnvironments to imported JobQueue '${id}'`,
          this,
        );
      }
    }
    ...
    return new Import(scope, id, {
      environmentFromArn: jobQueueArn,
    });
  }

  public readonly resource: batchJobQueue.BatchJobQueue;
  ...
  constructor(scope: Construct, id: string, props: JobQueueProps = {}) {
    ...
    // TERRACONSTRUCTS DEVIATION: the `aws_batch_job_queue` Terraform resource requires `name` up
    // front (no `name_prefix` support, unlike e.g. SQS queues), so unlike upstream's
    // CloudFormation-generated physical name, an omitted `jobQueueName` resolves to a
    // stack-scoped unique name here.
    const jobQueueName =
      props.jobQueueName ??
    ...
    this.resource = new batchJobQueue.BatchJobQueue(this, "Resource", {
      name: jobQueueName,
      // NOTE: the `xxxToTerraform` conversion (camelCase -> snake_case) that the generated
      // `BatchJobQueue._toTerraform()` normally applies via `cdktn.listMapper()` only runs
      ...
      computeEnvironmentOrder: Lazy.anyValue({
        produce: () =>
          this.computeEnvironments.map((ce) =>
            batchJobQueue.batchJobQueueComputeEnvironmentOrderToTerraform({
      ...
      jobStateTimeLimitAction: this.renderJobStateTimeLimitActions(
        props.jobStateTimeLimitActions,
      ),
    });

    this.jobQueueArn = this.resource.arn;
    this.jobQueueName = this.resource.name;

    this.node.addValidation({
      validate: () =>
        validateOrderedComputeEnvironments(this.computeEnvironments),
    });
  }
...
function validateOrderedComputeEnvironments(
  ...
  return seenOrders.length === 0
    ? ["This JobQueue does not link any ComputeEnvironments"]
    : [];
}
