// https://github.com/aws/aws-cdk/blob/v2.175.1/packages/@aws-cdk/aws-ec2-alpha/lib/vpc-v2.ts
...
import { DefaultInstanceTenancy, ISubnet, SubnetType } from "./vpc";
import { Arn } from "../arn";
...
export class VpcV2 extends VpcV2Base {
   ...
   */
  public static fromVpcV2Attributes(
    scope: Construct,
    id: string,
        ...
        if (props.secondaryCidrBlocks) {
          for (const cidrBlock of props.secondaryCidrBlocks) {
            if (cidrBlock.ipv4IpamProvisionedCidrs) {
              this.ipv4IpamProvisionedCidrs.push(
                ...cidrBlock.ipv4IpamProvisionedCidrs,
              );
            }
          }
        }
   ...
   */
  public readonly resource: vpc.Vpc;
  ...
  constructor(scope: Construct, id: string, props: VpcV2Props = {}) {
    ...
    this.ipAddresses =
      props.primaryAddressBlock ?? IpAddresses.ipv4("10.0.0.0/16");
    ...
    // TODO: Additional cidr blocks?
    this.ipv6CidrBlocks = [this.resource.ipv6CidrBlock];
    ...
    if (props.secondaryAddressBlocks) {
      const secondaryAddressBlocks: IIpAddresses[] =
        props.secondaryAddressBlocks;

      for (const secondaryAddressBlock of secondaryAddressBlocks) {
        const secondaryVpcOptions: VpcCidrOptions =
          secondaryAddressBlock.allocateVpcCidr();
        if (!secondaryVpcOptions.cidrBlockName) {
          throw new Error(
            "Cidr Block Name is required to create secondary IP address",
          );
        }

        if (
          secondaryVpcOptions.amazonProvided ||
          secondaryVpcOptions.ipv6IpamPool
        ) {
          this.useIpv6 = true;
        }
        ...
        if (secondaryVpcOptions.ipv4CidrBlock!) {
          ...
          if (ret === false) {
            throw new Error(
              "CIDR block should be in the same RFC 1918 range in the VPC",
            );
          }
        }
        if (secondaryVpcOptions.ipv4IpamProvisionedCidrs!) {
          this.ipv4IpamProvisionedCidrs?.push(
            ...secondaryVpcOptions.ipv4IpamProvisionedCidrs,
          );
        }
        const vpcCidrBlock = new VPCCidrBlock(
          this,
          secondaryVpcOptions.cidrBlockName,
          {
            vpcId: this.vpcId,
...
class IpamIpv6 implements IIpAddresses {
  ...
  allocateVpcCidr(): VpcCidrOptions {
    return {
      ...
      dependencies: this.props.ipamPool?.ipamCidrs.map(
        (c) => c as TerraformResource,
      ),
      cidrBlockName: this.props.cidrBlockName,
    };
  }
 ...
 *
 * See [VPC CIDR blocks](https://docs.aws.amazon.com/vpc/latest/userguide/vpc-cidr-blocks.html)
 * @internal
 */
class VPCCidrBlock extends AwsConstructBase implements IVPCCidrBlock {
  /**
  ...
  public static fromVPCCidrBlockattributes(
    ...
    class Import extends AwsConstructBase implements IVPCCidrBlock {
  ...
  constructor(scope: Construct, id: string, props: VPCCidrBlockProps) {
    super(scope, id);
    const isIpv6 =
      props.amazonProvidedIpv6CidrBlock === true ||
      props.ipv6IpamPoolId != undefined;
    this.resource = isIpv6
      ? new vpcIpv6CidrBlockAssociation.VpcIpv6CidrBlockAssociation(this, id, {
          vpcId: props.vpcId,
          assignGeneratedIpv6CidrBlock: props.amazonProvidedIpv6CidrBlock,
          ipv6IpamPoolId: props.ipv6IpamPoolId,
          ipv6NetmaskLength: props.ipv6NetmaskLength,
          // ipv6Pool: ??,
          // ipv6CidrBlock: ???,
        })
      : new vpcIpv4CidrBlockAssociation.VpcIpv4CidrBlockAssociation(this, id, {
          vpcId: props.vpcId,
...
function validateIpv4address(cidr1?: string, cidr2?: string): boolean {
  ...
  const octetsCidr1: number[] = cidr1
    .split(".")
    .map((octet) => parseInt(octet, 10));
  const octetsCidr2: number[] = cidr2
    .split(".")
    .map((octet) => parseInt(octet, 10));
