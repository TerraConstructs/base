 ...
 */
export class SqsEventSource implements compute.IEventSource {
  ...
  public bind(target: compute.IFunction) {
    const eventSourceMapping = target.addEventSourceMapping(
      `SqsEventSource:${AwsStack.uniqueId(this.queue.node)}`,
      {
        ...
        // filterEncryption: this.props.filterEncryption, // TODO: Re-Add KMS support
      },
    );
    ...
    if (target.role) {
      ...
      Annotations.of(target).addWarning(
