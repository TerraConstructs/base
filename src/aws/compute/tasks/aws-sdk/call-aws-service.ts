 ...
 */
export class CallAwsService extends compute.TaskStateBase {
  ...
  constructor(
    scope: Construct,
    id: string,
    private readonly props: CallAwsServiceProps,
  ) {
    super(scope, id, props);

    if (this.props.integrationPattern === compute.IntegrationPattern.RUN_JOB) {
      throw new Error(
    ...
    if (this.props.parameters) {
      ...
      if (invalidKeys.length) {
        throw new Error(
          `parameter names must be PascalCase, got: ${invalidKeys.join(", ")}`,
        );
      }
    }
  ...
  protected _renderTask(): any {
    ...
    return {
      ...
      Parameters: compute.FieldUtils.renderObject(this.props.parameters) ?? {}, // Parameters is required for aws-sdk
    };
  }
}
