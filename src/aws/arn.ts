...
export class Arn {
  ...
  public static format(components: ArnComponents, stack?: IAwsStack): string {
    ...
    const sep =
      components.sep ??
      (components.arnFormat === ArnFormat.COLON_RESOURCE_NAME ? ":" : "/");
    ...
    if (sep !== "/" && sep !== ":" && sep !== "") {
      throw new Error(
        'resourcePathSep may only be ":", "/" or an empty string',
      );
    }
  ...
  public static split(arn: string, arnFormat: ArnFormat): ArnComponents {
    ...
    const [, partition, service, region, account, resourceTypeOrName, ...rest] =
      components;
    ...
    if (slashIndex !== -1) {
      resource = resourceTypeOrName.substring(
        resourcePartStartIndex,
        slashIndex,
      );
  ...
  public static extractResourceName(arn: string, resourceType: string): string {
    ...
    if (!parsed.resourceName) {
      throw new Error(
        `Expected resource name in ARN, didn't find one: '${arn}'`,
      );
    }
...
function parseArnShape(arn: string): "token" | string[] {
  ...
  if (!looksLikeArn) {
    if (cdktf.Token.isUnresolved(arn)) {
      ...
      throw new Error(
        `ARNs must start with "arn:" and have at least 6 components: ${arn}`,
      );
    }
  }
  ...
  if (!partition) {
    throw new Error(
      "The `partition` component (2nd component) of an ARN is required: " + arn,
    );
  }
  ...
  if (!service) {
    throw new Error(
      "The `service` component (3rd component) of an ARN is required: " + arn,
    );
  }
  ...
  if (!resource) {
    throw new Error(
      "The `resource` component (6th component) of an ARN is required: " + arn,
    );
  }
