...
export class FilterPattern {
  ...
  public static all(...patterns: JsonPattern[]): JsonPattern {
    if (patterns.length === 0) {
      throw new Error(
        "Must supply at least one pattern, or use allEvents() to match all events.",
      );
    }
...
class TextLogPattern implements IFilterPattern {
  ...
  constructor(clauses: string[][]) {
    const quotedClauses = clauses.map((terms) =>
      terms.map(quoteTerm).join(" "),
    );
...
export class SpaceDelimitedTextPattern implements IFilterPattern {
  ...
  public whereString(
    ...
    if (this.columns.indexOf(columnName) === -1) {
      throw new Error(
        `Column in restrictions that is not in columns: ${columnName}`,
      );
    }
  ...
  public get logPatternString(): string {
    return (
      "[" + this.columns.map(this.columnExpression.bind(this)).join(", ") + "]"
    );
  }
...
function validateStringOperator(operator: string) {
  ...
  if (operator !== "=" && operator !== "!=") {
    throw new Error(
      `Invalid comparison operator ('${operator}'), must be either '=' or '!='`,
    );
  }
