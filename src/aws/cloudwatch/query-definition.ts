...
export class QueryString {
  ...
  public toString(): string {
    return [
      ...
      .filter((queryLine) => queryLine !== undefined && queryLine.length > 0)
      .join("\n| ");
  }
  ...
  private buildQueryLines(command: string, statements?: string[]): string[] {
    ...
    return statements.map((statement: string): string =>
      this.buildQueryLine(command, statement),
    );
  }
...
export class QueryDefinition extends AwsConstructBase {
  ...
  public get queryDefinitionOutputs(): QueryDefinitionOutputs {
    return {
      queryDefinitionId: this.queryDefinitionId,
    };
  }
  public get outputs(): Record<string, any> {
    return this.queryDefinitionOutputs;
  }

  public readonly resource: cloudwatchQueryDefinition.CloudwatchQueryDefinition;

  constructor(scope: Construct, id: string, props: QueryDefinitionProps) {
    super(scope, id, props);
    const name =
      props.queryDefinitionName ||
      this.stack.uniqueResourceName(this, {
        prefix: this.gridUUID,
      });

    this.resource = new cloudwatchQueryDefinition.CloudwatchQueryDefinition(
      this,
      "Resource",
      {
        name,
    ...
    this.queryDefinitionId = this.resource.queryDefinitionId;
  }
}
