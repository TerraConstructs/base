// https://github.com/aws/aws-cdk/blob/v2.170.0/packages/aws-cdk-lib/aws-cloudwatch/lib/private/statistic.ts
...
function parsePairStatistic(
  ...
  const parseNumberAndPercent = (
    x: string,
  ): [number | undefined | "fail", boolean] => {
    x = x.trim();
    if (!x) {
  ...
  const asSingleStatStr = canBeSingleStat
    ? `${prefix.toLowerCase()}${upper}`
    : undefined;
...
export function parseStatistic(
  ...
  let m:
    | ReturnType<typeof parseSingleStatistic>
    | ReturnType<typeof parsePairStatistic> = undefined;
...
export function normalizeStatistic(
  parsed: ReturnType<typeof parseStatistic>,
): string {
