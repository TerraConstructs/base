// https://github.com/aws/aws-cdk/blob/v2.170.0/packages/aws-cdk-lib/aws-cloudwatch/lib/private/rendering.ts

import { DropEmptyObjectAtTheEndOfAnArray } from "./drop-empty-object-at-the-end-of-an-array-token";
import {
  accountIfDifferentFromStack,
...
export function allMetricsGraphJson(left: IMetric[], right: IMetric[]): any[] {
  ...
  return mset.entries.map(
    (entry) =>
      new DropEmptyObjectAtTheEndOfAnArray(
        metricGraphJson(entry.metric, entry.tag, entry.id),
      ),
  );
}
...
function metricGraphJson(metric: IMetric, yAxis?: string, id?: string) {
  ...
  dispatchMetric(metric, {
    withStat(stat) {
      ...
      // Dimensions
      for (const [dimName, dimValue] of Object.entries(stat.dimensions || {})) {
        ret.push(dimName, dimValue);
      }
...
export class MetricSet<A> {
  ...
  private addOne(metric: IMetric, tag?: A, id?: string) {
    ...
    if (conf.mathExpression) {
      for (const [subId, subMetric] of Object.entries(
        conf.mathExpression.usingMetrics,
      )) {
