...
export function metricKey(metric: IMetric): string {
  ...
  if (conf.metricStat) {
    ...
    for (const [dimName, dimValue] of Object.entries(
      conf.metricStat.dimensions || {},
    )) {
      parts.push(dimName);
      parts.push(dimValue);
    }
...
export function dispatchMetric<A, B>(
  ...
  if (conf.metricStat && conf.mathExpression) {
    throw new Error(
      "Metric object must not produce both 'metricStat' and 'mathExpression'",
    );
    ...
    throw new Error(
      "Metric object must have either 'metricStat' or 'mathExpression'",
    );
  }
}
