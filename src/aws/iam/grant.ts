...
export class Grant implements IDependable {
   ...
   *   resource construct.
   */
  public static addToPrincipalOrResource(
    ...
    const equalOrBothUnresolved =
      resourceAndPrincipalAccountComparison === TokenComparison.SAME ||
      resourceAndPrincipalAccountComparison == TokenComparison.BOTH_UNRESOLVED;
  ...
  public static addToPrincipal(options: GrantOnPrincipalOptions): Grant {
    ...
    const addedToPrincipal =
      options.grantee.grantPrincipal.addToPrincipalPolicy(statement);
   ...
   */
  public static addToPrincipalAndResource(
  ...
  private constructor(props: GrantProps) {
    ...
    Dependable.implement(this, {
      get dependencyRoots() {
        return Array.from(
          new Set(
            self.dependables.flatMap((d) => Dependable.of(d).dependencyRoots),
          ),
        );
      },
    });
  ...
  public get success(): boolean {
    return (
      this.principalStatement !== undefined ||
  ...
  public combine(rhs: Grant) {
    ...
    ret.principalStatements.splice(
      0,
      ret.principalStatements.length,
      ...combinedPrinc,
    );
    ret.resourceStatements.splice(
      0,
      ret.resourceStatements.length,
      ...combinedRes,
    );
