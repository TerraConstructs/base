// https://github.com/aws/aws-cdk/blob/v2.263.0/packages/aws-cdk-lib/aws-rds/lib/props.ts
...
// TERRACONSTRUCTS DEVIATION: upstream types password/secret-value fields as `core.SecretValue`
// (a CloudFormation dynamic-reference wrapper), which is not ported in this repo (see the
// TERRACONSTRUCTS DEVIATION note on `SecretProps.secretObjectValue` in `../../encryption/secret.ts`).
// Every `SecretValue` field below is a plain `string` instead (which may itself be an unresolved
// Token).
...
export interface CredentialsBaseOptions {
   ...
   */
  readonly replicaRegions?: encryption.ReplicaRegion[];
}
...
export abstract class Credentials {
   ...
   * Do not put passwords in your CDK code directly.
   */
  public static fromPassword(username: string, password: string): Credentials {
    return {
      username,
      password,
      usernameAsString: true,
  ...
  // TODO: omitted — depends on `ISecret.secretValueFromJson`, which is not ported in this repo (see
  // the TERRACONSTRUCTS DEVIATION note on `ISecret` in `../../encryption/secret.ts`: there is no
  // Terraform-native way yet to reference a JSON sub-field of an existing secret as a token).
  // Reinstate once that capability lands.
  // — https://github.com/aws/aws-cdk/blob/v2.263.0/packages/aws-cdk-lib/aws-rds/lib/props.ts#L233-L258
  // /**
   ...
   * @default - default master key
   */
  public abstract readonly encryptionKey?: encryption.IKey;
   ...
   * @default - none
   */
  public abstract readonly secret?: encryption.ISecret;
   ...
   * @default - Secret is not replicated
   */
  public abstract readonly replicaRegions?: encryption.ReplicaRegion[];
}
...
export abstract class SnapshotCredentials {
   ...
   */
  public static fromPassword(password: string): SnapshotCredentials {
  ...
  // Terraform-native way yet to reference a JSON sub-field of an existing secret as a token).
  // Reinstate once that capability lands.
  // — https://github.com/aws/aws-cdk/blob/v2.263.0/packages/aws-cdk-lib/aws-rds/lib/props.ts#L389-L406
  // /**
  ...
  //  * ```
  //  */
  // public static fromSecret(secret: encryption.ISecret): SnapshotCredentials {
  //   return {
...
export interface RotationMultiUserOptions extends CommonRotationUserOptions {
   ...
   * ```
   */
  readonly secret: encryption.ISecret;
}
