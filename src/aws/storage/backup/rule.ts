// https://github.com/aws/aws-cdk/blob/v2.263.0/packages/aws-cdk-lib/aws-backup/lib/rule.ts
...
export interface BackupPlanRuleProps {
   ...
   * @default - no schedule
   */
  readonly scheduleExpression?: notify.Schedule;
...
export class BackupPlanRule {
  ...
  constructor(props: BackupPlanRuleProps) {
    if (
      props.deleteAfter &&
      !props.deleteAfter.isUnresolved() &&
      props.moveToColdStorageAfter &&
      !props.moveToColdStorageAfter.isUnresolved() &&
      props.deleteAfter.toDays() < props.moveToColdStorageAfter.toDays()
    ) {
      throw new UnscopedValidationError(
        "`deleteAfter` must be greater than `moveToColdStorageAfter`",
      );
    }

    if (
      props.scheduleExpression &&
      !/^cron/.test(props.scheduleExpression.expressionString)
    ) {
      throw new UnscopedValidationError(
        "`scheduleExpression` must be of type `cron`",
      );
    }

    const deleteAfter =
      props.enableContinuousBackup && !props.deleteAfter
        ? Duration.days(35)
        : props.deleteAfter;
    ...
    if (props.copyActions && props.copyActions.length > 0) {
      props.copyActions.forEach((copyAction) => {
        if (
          ...
          !copyAction.deleteAfter.isUnresolved() &&
          copyAction.moveToColdStorageAfter &&
          !copyAction.moveToColdStorageAfter.isUnresolved() &&
          copyAction.deleteAfter.toDays() <
            copyAction.moveToColdStorageAfter.toDays() + 90
        ) {
          throw new UnscopedValidationError(
            [
