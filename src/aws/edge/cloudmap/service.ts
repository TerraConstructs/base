// https://github.com/aws/aws-cdk/blob/v2.233.0/packages/aws-cdk-lib/aws-servicediscovery/lib/service.ts
...
export interface IService extends IAwsConstruct {
  ...
  readonly serviceOutputs: ServiceOutputs;
...
export interface ServiceProps extends DnsServiceProps, AwsConstructProps {
  /**
...
abstract class ServiceBase extends AwsConstructBase implements IService {
  public get serviceOutputs(): ServiceOutputs {
    return {
...
export class Service extends ServiceBase {
  ...
  public static readonly PROPERTY_INJECTION_ID: string =
    "terraconstructs.aws.edge.cloudmap.Service";
  ...
  public static fromServiceAttributes(
    ...
    class Import extends ServiceBase {
      ...
      public discoveryType =
        attrs.discoveryType || defaultDiscoveryType(attrs.namespace);
    }
   ...
   */
  public readonly resource: serviceDiscoveryService.ServiceDiscoveryService;
  ...
  constructor(scope: Construct, id: string, props: ServiceProps) {
    ...
    const discoveryType =
      props.discoveryType || defaultDiscoveryType(props.namespace);
    ...
    if (props.healthCheck && props.customHealthCheck) {
      throw new ValidationError(
        "Cannot specify both `healthCheckConfig` and `healthCheckCustomConfig`.",
        this,
      );
    }
    ...
    if (
      ...
      throw new ValidationError(
        "Cannot specify `resourcePath` when using a `TCP` health check.",
        this,
      );
    }
    ...
    const healthCheckConfigDefaults = {
      ...
      failureThreshold: 1,
      resourcePath:
        props.healthCheck && props.healthCheck.type !== HealthCheckType.TCP
          ? "/"
          : undefined,
    };
    ...
    // Create service
    this.resource = new serviceDiscoveryService.ServiceDiscoveryService(
      this,
      "Resource",
      {
        // TERRACONSTRUCTS DEVIATION: the provider's `name` is required (no name_prefix),
        // while upstream CfnService.name is optional (CloudFormation-generated). Fall back
        // to the stack-scoped exact-name form per the naming convention.
        name: props.name ?? this.stack.uniqueResourceName(this),
    ...
    this.serviceId = this.resource.id;
