// https://github.com/aws/aws-cdk/blob/v2.232.2/packages/aws-cdk-lib/core/lib/errors.ts

import type { IConstruct } from "constructs";

// TODO: check whether Assertion and CloudAssembly errors are needed
const CONSTRUCT_ERROR_SYMBOL = Symbol.for(
  "terraconstructs/core.SynthesisError",
);
const VALIDATION_ERROR_SYMBOL = Symbol.for(
  "terraconstructs/core.ValidationError",
);
const ASSUMPTION_ERROR_SYMBOL = Symbol.for(
  "terraconstructs/core.AssumptionError",
);
const EXECUTION_ERROR_SYMBOL = Symbol.for(
  "terraconstructs/core.ExecutionError",
);
...
abstract class ConstructError extends Error {
  ...
  constructor(msg: string, scope?: IConstruct, name?: string) {
    ...
    if (scope) {
      ...
      // TODO: maybe something similar to "constructInfoFromConstruct"
      this.#constructInfo = undefined;
    }
  ...
  private constructStack(prev?: string) {
    ...
    if (this.constructInfo) {
      stack.push(
        `${indent}at path [${this.constructPath}] in ${this.constructInfo?.fqn}`,
      );
    } else {
